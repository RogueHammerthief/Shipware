...
{
    //ARCHIVE: Created just after expanding the functionallity of AutoPopulate, and just before 
    //replacing the old Action-based handlers with bespoke TallyHandler objects.
    ...
    partial class Program : MyGridProgram
    {
        ...
        public string populatePB(string pbConfig, MyIni configReader, out string outcome)
        {
            ...
            //Our life will be much easier if we rebuild the Tally dictionaty, so that's what we'll do.
        ...
        public string writeTemplate(MyCommandLine argReader, StringBuilder _sb, out bool isSuccessful)
        {
            ...
            else if (type == "actionsectionterminal")
            {
                ...
                _sb.Append(
                    ";  The value that will be applied to this propery when this ActionSet \n" +
                    ";  is set to 'on'.\n");
                _sb.Append(
            ...
            else if (type == "actionset")
            {
                ...
                _sb.Append(
                    ...
                    ";  'Action {set name} {command} -{reply target}, where 'reply target' is\n" +
                    ";  the ScriptTag of this script instance. As an example:\n" +
            ...
            else if (type == "trigger")
            {
                ...
                _sb.Append(
                    ...
                    ";  percentage of the taget Tally meets or falls below the LessOrEqualValue.\n");
                _sb.Append(
                ...
                _sb.Append(
                    ";  The command that will be sent to the operated ActionSet when the\n" +
                    ";  percentage of the taget Tally meets or exceeds the GreaterOrEqualValue.\n");
                _sb.Append(
        ...
        public string autoPopulate(IMyGridTerminalSystem GTS, ref List<Tally> tallyList, MyIni configWriter)
        {
            ...
            //The config we apply to Cargo blocks will change, based on the capabilities of the grid.
            string cargoConfig = $"[{tag}]\nTallies = Cargo";
            ...
            //If a gatling weapon is present, we need to update the cargo config
            if (gatlings.Count > 0)
            { cargoConfig = $"{cargoConfig}, Drums"; }
            ...
            if (rockets.Count > 0)
            { cargoConfig = $"{cargoConfig}, Rockets"; }
            //H2/02 generators.
            ...
            { cargoConfig = $"{cargoConfig}, Ice"; }
            //Drills. These will be used for both the Ore and Stone tallies, so we'll store them as
            //simple TerminalBlocks
            List<IMyTerminalBlock> drills = new List<IMyTerminalBlock>();
            GTS.GetBlocksOfType<IMyShipDrill>(drills, b => b.IsSameConstructAs(Me));
            //Refineries will also be used for Ore and Stone.
            ...
            if (batteries.Count > 0) //Really, what are the odds of not having a battery?
            {
                ...
                {
                    newTally = new TallyGeneric(meterMaid, tallyName, handleBattery);
                    tallyList.Add(newTally);
                    tallyCounter++;
                }
            ...
            if (hyTanks.Count > 0)
            {
                ...
                if (!tallyDic.ContainsKey(tallyName))
                {
                    newTally = new TallyGeneric(meterMaid, tallyName, handleGas);
                    tallyList.Add(newTally);
                    tallyCounter++;
                }
            ...
            if (smallPanels.Count > 0 || largePanels.Count > 0)
            {
                ...
                if (!tallyDic.ContainsKey(tallyName))
                {
                    ...
                    newTally = new TallyGeneric(meterMaid, tallyName, handlePowerProducer, false, 1000);
