...
{
    //ARCHIVE: Created just prior to replacing block-type grid evaluation with tally-type grid 
    //evaluation.
    ...
    ///<status> <date>202010105</date>
    ///  <content>
    ///     Today, I think I'll try something new. Instead of just chucking something in and seeing 
    ///   how it breaks, I'm going to try planning. This scenario will have the battery sorter at 
    ///   the top of the list, followed by PowerProvider and GasTank (Alternatively: the reason 
    ///   inventory is at the bottom of the list is so I can try everything else first. If I have
    ///   a system that's actually allowing me to send more information downstream than 'I know what
    ///   kind of block this is', should I do the inventory sorter first?)
    ///     What about a 'registered' flag that gets set when a tally is successfully created?
    ///     Okay, new scenario. PB sorter is at the top of the list, followed by inventory, followed
    ///   by the rest in no particular order (Except that the battery sorter comes before the 
    ///   PowerProvider sorter). A 'registered' flag is added to the implementation, that starts
    ///   out false and gets set to true right after a tally (Or whatever sort of data structure
    ///   the sorter wants to create) is successfully created. All other sorters check the 
    ///   'registered' flag before they decide to run. EvaluateGeneric and EvaluateReport are 
    ///   modified to return suppressedErrors (Somehow. EvaluateReport currently returns a Report.)
    ///   (Note: If I go with this, it might be a good opportunity to replace the duplicate code in
    ///   the inventory sorter with a specialized method that gets called twice).
    ///     Scenario 1: Hydrogen tank wants to implement a Gas tally.
    ///     -Hydrogen tank is not a PB, and passes the PB sorter
    ///     -Hydrogen tank has an inventory, and gets caught by the inventory sorter. The handled 
    ///    flag is set (Maybe. That's not how the current sorter works, but I'd probably change that
    ///    if I moved it to the top of the list.). A Tallies key is detected, the name of the gas 
    ///    tally is matched to its Tally object, the Tally is found to not be a CargoTally. A 
    ///    suppressedError is logged and the registered flag remains false.
    ///     -Hydrogen tank passes other sorters because it's not their type.
    ///     -Hydrogen tank gets caught by the IMyGasTank sorter. The handled flag is set, again. A
    ///    Tallies key is detected, the name of the gas tally is matched to a Tally object, the 
    ///    Tally is found to have a compatible currHandler. The Tally is added to genericRefs. The
    ///    registered flag is set to true? At the end of the sorter, every tally in genericRefs 
    ///    (Just the gas tally, in this case) gets this block added to their internal list, and 
    ///    their max incremented.
    ///     -All other sorters are bypassed because the registered flag is set.
    ///     Scenario 2: Hydrogen tank wants to implement a Cargo tally
    ///     Scenario 3: Hydrogen tank wants to implement both a Gas and a Cargo tally.
    ///     Not even going to try the other scenarios, because I already see the problem: I may have
    ///   multiple tallies on a block, but only one 'registered' flag. When it comes time to try a
    ///   gas and cargo tally, the inventory sorter will catch the cargo tally and the gas sorter
    ///   won't even fire.
    ///     Maybe I could do something with the suppressedErrors string? But it's just a string, I 
    ///   can't tell why an error was thrown, or how many were thrown.
    ///     Should I be trying to read the Tallies string before trying any of the sorters? Then I
    ///   could have a 'registered' flag for each one. But then how would I deal with surfaces, 
    ///   indicators, MFDs, and whatever else?
    ///     Should I arrange the sorters by what kind of keys they have? If a block has the 
    ///   Tallies key, it goes to one set of sorters, if it has the Surface0 key, it goes to a 
    ///   different set? But then how do I deal with cockpits, which may have both? And 
    ///   SurfaceProviders that only want something displayed on Surface3?
    ///     Should I explicitly state in the configuration what types of sorters to subject this
    ///   block to? 'Type = Surface, Tallies, etc'. But that'd significantly increase the ammount
    ///   of data you'd have to enter per block, especially in the case of things like Batteries,
    ///   which are probably only members of one tally.
    ///     Originally, I had been quite pleased with my decision to tackle the hardest portion of
    ///   this script first. But I think starting with just one tally type and basing all of my 
    ///   designs around how to make that one work is coming back to bite me, now.
    ///   
    ///     Okay, what if I check for a Tallies key first (If the block has an inventory, checks 
    ///   can be made for Tallies0, Tallies1, etc). An attempt is made to match Tally names to Tally
    ///   objects, matching tally objects are added to a list, complaints are filed as nessecary. 
    ///   The Tally list is actually a KeyValuePair(Tally,bool). There is a set of sorters for each
    ///   tally type (Cargo or item, Generic-power-gas-output etc). When the tally is found to match
    ///   a certain tally type, the block type is checked for compatability. If that check clears,
    ///   the block is added to the tally's internal listing, its max is incremented, and the bool
    ///   associated with this tally in the list is set to true. If that check fails, an error is 
    ///   added to the error string and the bool remains false.
    ///     At the end of the tally sorting loop, the list is looped through again, this time 
    ///   checking the bools. When one is false, an error is thrown (... Though, what kind of error?
    ///   How would you reach this state without throwing some other error along the way? I think 
    ///   this is where I'd throw a 'block type can't be handled by this script' error in the 
    ///   current setup, but with this, I think all these instances will be caught by the 'tally
    ///   incompatible with block type' error. Oh, worth noting: I think I'll be able to have a 
    ///   if/else if/else setup for this portion, for zero possibility of leaks.).
    ///     After the Tally sorters, the block is checked by the remaining existing block type 
    ///   sorters: LightingBlock, SurfaceProvider, probably Camera since raycast is going to be such
    ///   a special snowflake. These function in basically the same way that they do now.
    ///  </content>
    ///</status>

    ///<status> <date>202010104</date>
    ///  <content>
    ///     So, what's up for today?
    ///     While it's true that rgbColorFromString could use some improvment, it's only used during
    ///   evaluation, and then only for surfaces, and probably only for MFDs at that. And it works.
    ///   I think that'll suffice, for the time being. 
    ///     As they stand, my outstanding issues are the reactor cargos, and... well, that's it 
    ///   really. I think I may add an optional argument to evaluateGeneric just for this case 
    ///   (Well, this and the earlier one with gas tanks) that allows me to add a condition to the
    ///   block type check, so I can make them fail silently if they have an inventory.
    ///     After that, implementations for GameScripts (I'll tempt fate and suggest that 
    ///   should be pretty easy) and TextWall MFD pages, with the latter being tuned towards being
    ///   able to report a raycaster's results (Maybe with a 'DataSource' field that can take either
    ///   a raycaster object or the name of a block, with the latter used for the 'database' idea I
    ///   had for writing help text and the like into a TimerBlock's custom data). Then I'll start
    ///   work on the raycaster itself. Or should I do that first, so I have an idea of how to get
    ///   the raycasters and the TextWalls to talk to each other?
    ///     On the inconveinent inventories: What would the effect be if I set the 'handled' flag 
    ///   to false on those occasions where a block that can be handled by one tally type also has
    ///   an inventory? And maybe move my two trouble sorters to the top of the list so they don't
    ///   interfere with the rest of my sorters? First problem is that Handled is set in the main 
    ///   thread; I'd need some method of passing that result out of evaluateReport.
    ///     For the record, I think my hesitance in making this change is the mountain of unforseen
    ///   consequences that could come with it. But what I'm thinking is that I'll go ahead and move
    ///   the Gas and PowerProvider sorters to the top of the list, then add a ignoreType = false 
    ///   parameter to EvaluateGeneric and flag it to true for those two sorters. Then, instead of 
    ///   just manually setting handled before calling evaluateGeneric, I'll have that method return
    ///   a boolean that is always true... except if it tries to throw a type error when ignoreType
    ///   is true.
    ///     Okay, this... seems to work? I would've expected it not to, honestly. I thought for sure
    ///   that checking for PowerProvider before BatteryBlock would cause an issue.
    ///     Tried: Adding an oxygen tank to both an oxygen and cargo tally. Failed with an inventory
    ///   block type error.
    ///     Tried: Adding a Solar Panel to a Batteries tally. Got a 'Block type cannot be handled by
    ///   script' error.
    ///     Tried: Adding a Battery to a PowerProvider tally. Got a 'Block type incompatible with 
    ///   tally' error.
    ///     Bloody edge cases. And I probably didn't think of everything. Maybe manipulating the 
    ///   handled flag isn't the answer, here...
    ///     What if I had a 'supressedErrors' string, in addition to the normal string? Then, 
    ///   instead of messing with 'handled', I could just put errors when type is ignored into the
    ///   suppressedErrors string, and only cart them out if we get to the end of evaluation with
    ///   no idea what to do with the block.
    ///  </content>
    ///</status>

    ///<status> <date>20210103</date>
    ///  <content>
    ///     For the damage display: It'd be best if the name listing were alphabetized. I haven't
    ///   decided yet if I'll split damaged and destroyed blocks into seperate lists: That would be
    ///   easier, but having all of them in one sprite-based, color-coded list would probably make
    ///   damage control easier. I need to remember to have an 'exclude string from damage report'
    ///   (NNF) key on the PB so I can avoid having things like the ship name in every single entry.
    ///   I think I'll have a /very/ lazy implementation of cutting strings to length that simply
    ///   cuts them in half if they're too big, and declares success if that makes them fit.
    ///     I can add 'add page' and 'remove page' methods to the MFD for the damage report to call
    ///   on the fly if it needs more pages to display its results. It'd probably be best if there
    ///   were some sort of on-screen notification of where the scanner was in its crawl through the
    ///   grid (Block 240 of 300). I guess I could put the damage report scanner on a seperate 
    ///   update frequency, like 10. That would spread the work out, though I'd need to be calling
    ///   update() on the report constantly, and things would probably get a bit awkward when it
    ///   and the other update loop tried to refresh a screen in the same 10-tic span. But maybe
    ///   not noticibly so. Maybe add a 'hasFoucs' flag to Reports, that would always be true in
    ///   the case of other reports, but would allow me to decide if I need to call update() on a
    ///   particular page in an MFD? Or could I come at this from the other direction? Somehow only
    ///   call update on the MFD when a damage report page was up?
    ///     Should I allow an option to scan all functional blocks, so it could pick up on things
    ///   like damaged conveyors? But how would I display those? They don't have CustomNames, and
    ///   would therefore need special handling.
    ///     Can I come up with a clever method for figuring out if a block has been replaced, or
    ///   do I just need to rely on the user to re-scan the grid once they've got it fixed? Maybe
    ///   I could run a position check on destroyed blocks to see if something is now filling their
    ///   old position in the grid, and change the color to gray or something to reflect that?
    ///     Info: Saw this comment in the Discord, which may be relevent to a damage report 
    ///   implementation:
    ///     hey whip - saw your simpl script in the workshop and started disecting it - I thought 
    ///   you couldnt get armour blocks from the pb, but im seeing Me.CubeGrid.CubeExists - does 
    ///   the script iterate x, y, z on the grid and check that a non-computer block occupies a 
    ///   space?
    ///  </content>
    ///</status>

    ///<status> <date>202110102</date>
    ///  <content>
    ///     Wonder if there's any way I could make the number of pages on an MFD dynamic? I mean,
    ///   the pages are in a Dictionary, so obviously I can extend it at any time. I guess I'm just
    ///   trying to figure out how I could allow the DamageReport function to add additional pages 
    ///   if it needs more room to display all the damaged blocks.
    ///     It didn't occur to me how involved it was going to be to set up colors. But, I think 
    ///   I've got something functional. 's messy, though.
    ///     Okay, tested the new implementation of evaluateReport. Seems to be working, or at least
    ///   it is after I tracked down a few minor issues. Wonder how difficult it'll be to get the 
    ///   Script and WallOText Reportables working?
    ///  </content>
    ///</status>

    ///<status> <date>20210101</date>
    ///  <content>
    ///     Been thinking.
    ///     First thing: I may make the Reports array store IReportable instead, and add the MFDs
    ///   to it as well as their dictionary. I think there's some overhead associated with getting
    ///   Dictionary.Values, and this would avoid it. And anyway, this is /why/ I made MFDs 
    ///   implement IReportable in the first place.
    ///     I also need to go ahead and implement a way to set fore and back colors on reports.
    ///   While this would mostly be uesd in MFDs, there's no reason not to let it spill over
    ///   into regular reports.
    ///     I'm kind of thinking about re-working Raycasters into their own, self-contained object
    ///   that can be referenced like MFDs and that implements the IHasElement interface. That's 
    ///   the method I could use to link various reportables to a Raycaster: The name, followed by
    ///   a field (Example: Surface0Tallies = ForwardCaster.Charge). It'd be a pain to implement
    ///   (Especially, as always, in grid evaluation), but it'd actually potentially let me get any
    ///   piece of information collected by the Raycaster and display it wherever I want, as 
    ///   opposed to just in the wall of text that I've been using.
    ///     Of course, in order to generate a waypoint, I'd need something using WriteText.
    ///     While I'm figuring out how to get a report from a Raycaster, I should figure out how to
    ///   paste the event log into the same object type. I don't think I could ever get it to work
    ///   for un-handled exceptions, but it'd still be handy to have under normal circumstances.
    ///     Actually, this would probably work for the DamageReport as well. If I end up doing that.
    ///     As long as I'm taking stock (Happy New Year!), I figure that my next step after I've
    ///   got MFDs functional is a proper Raycast implementation, then ActionSets, and finally the
    ///   support functions: Populate/Clear/Template, followed by AutoPopulate (And the Generic 
    ///   report that goes with it), ChangeTag, and maybe Nuke. At that point, I think I'll have a 
    ///   script worthy of release, and the rest can be added in after the fact, when and if I feel 
    ///   like it.
    ///     Oh, also, I need to fix the probable-issue with UpdateFrequency and figure out what to
    ///   do about reactors' inconveinent inventories. And I could start on figuring out how to 
    ///   streamline MFD and Report evaluation. Maybe have an evalReport() method, running once in 
    ///   the case of a regular report and looped in the case of an MFD?
    ///     After much soul searching, I've decided to declare a local variable to store an MFD while
    ///   I assign it to Reports and MFDs. I've also stripped out the update call to the MFDs 
    ///   dictionary, as the call to Reports should now handle it. And I added a UpdateFrequency.None
    ///   to an evaluation with errors, to stop any previous loops.
    ///     As for evalReports... What I'm thinking is passing in a prefix, which'll be Surface<#>
    ///   in the case of regular reports and <MFDName><#> in the case of MFDs. It'll return a report,
    ///   and I can pull the title from that, if it exists, to use in place of the prefix generated
    ///   for an MFD page. I'd also re-write all the error messages to use the prefix in place of
    ///   Surface{i} or w/e I'm using now. Lastly, this would be a fine opportunity to implement
    ///   configuring fore and back colors for reports.
    ///  </content>
    ///</status>
    ...
    ///     ...aaaaaaaaand I just hit the character limit. Guess it's time to do some pruning. On 
    ///   that note, it's time to preserve this (working) use of BlockDefinition:
    ...
    ///     Okay, that got me down to 86395.
    ///     So. First problem. Current implementation pitches a fit if you run an MFD command with
    ///   no further arguments.
    ///     Okay. Fixed that (Hooray, more complaining). Apart from some other small bugs that I've
    ///   already squashed... I think this works. I might go so far as to say it works /well/.
    ///     Now how the hell am I going to feed Raycast and other data into this?
    ///     Okay, spoke a bit too soon. I was going to make the MFD commands non-case-sensitive, but
    ///   I don't think I'll be able to, because the dictionary /is/ case sensitive, and that's the
    ///   same place I pass in a page name.
    ///     Also, I was passing the name of the MFD itself into the place I was supposed to be 
    ///   sending the page name.
    ///     Also, my earlier fears about PowerProducers that have inventories have come to pass. And
    ///   I can't just pass this one over like I did with bottle inventory on the GasTanks; 'how
    ///   much uranium do I have in my reactors' is one of the first things that people are going
    ///   to want to know.
    ///     ...Though, weirdly, the uranium tally citing the reactors generated an error... but the
    ///   script appears to be running?
    ///     Yeah, not sure what happened there. The stores report was up and showing the correct
    ///   ammount of uranium on the grid. Wonder if there's still something carrying over from the
    ///   old instance when you use the Evaluate command?
    ///     Come to think of it, if you evaluate the grid and it clears, the update frequency is 
    ///   set. If you evaluate /again/ and the grid fails, the update fequency very likely keeps
    ///   its old value.
    ...
    partial class Program : MyGridProgram
    {
                            ...
                            if (argReader.ArgumentCount == 3)
                            {
                                ...
                                if (MFDs.ContainsKey(MFDTarget))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        //the command...
                                        if (!MFDs[MFDTarget].setPage(MFDPageCommand))
                                        {
                                            //... Complain.
        ...
        public void evaluate()
        {
            ...
            //MFDs are special, though. We'll leave them in a dictionary.
            ...
            foreach (IMyTerminalBlock block in blocks)
            {
                ...
                //TEST CODE: Moved Power and Gas sorters to the top, with a provision for withdrawing
                //the 'handled' flag if they run into a type error.
                ...
                if (parseResult.Success && block is IMyPowerProducer)
                {
                    //Like the Raycaster, all PowerProducers have their maximums set manually.
                    handled = evaluateGeneric(ref errors, "Power Producer", b => { return 0; },
                        handlePowerOutput, block, iniReader, evalTallies, true);
                }

                //GasTank Sorter (Generic)
                if (parseResult.Success && block is IMyGasTank)
                {
                    handled = evaluateGeneric(ref errors, "Gas Tank", b => (double)(((IMyGasTank)b).Capacity),
                        handleGas, block, iniReader, evalTallies, true);
                }
                        ...
                        else if (iniReader.ContainsKey(SECTION_TAG, $"Surface{i}MFD"))
                        {
                            ...
                            for (int i = 0; i < ((IMyTextSurfaceProvider)block).SurfaceCount; i++)
                            {
                                ...
                                else if (iniReader.ContainsKey(SECTION_TAG, $"Surface{i}MFD"))
                                {
                                    ...
                                    //That should be all the data available for this MFD. We'll go ahead and
                                    //build it...
                                    MFD newMFD = new MFD(pages);
                                    //Then add it to reports, so it'll be updated...
                                    evalReports.Add(newMFD);
                                    //Then add it to MFDs, so we can control it.
                                    MFDs.Add(MFDName, newMFD);
                                }
                            }*/
                        }
                ...
                if (parseResult.Success && block is IMyBatteryBlock)
                {
                    ...
                    handled = evaluateGeneric(ref errors, "Battery", b => (double)(((IMyBatteryBlock)b).MaxStoredPower), 
                ...
                {
                    handled = evaluateGeneric(ref errors, "Jump Drive", b => (double)(((IMyJumpDrive)b).MaxStoredPower),
                ...
                if (parseResult.Success && block is IMyCameraBlock)
                {
                    ...
                    handled = evaluateGeneric(ref errors, "Raycaster", b => { return 0; },
