...
{
    //ARCHIVE: Created just prior to attempting to build PopulatePB with the WriteConfig approach.
    //A partial implementation of the direct text editing approach is still present.
    ...
    partial class Program : MyGridProgram
    {
                    ...
                    switch (command)
                    {
                        ...
                        */
                        ...
                        case "Populate":
                            ...
                            {
                                //That is, if the PB even has a Populate section.
                                if (iniRead.ContainsSection("Populate"))
                                {
                            ...
                            //If the Populate section can't be read, do the user a solid and let them
                            //know why.
                            {
                        ...
                        //Copies the configuration from a 'Populate' section on the PB to members of
                        //the 'Populate' group on the grid
                        //Argument format: TemplatePop <TemplateType> <HowMany>
                        //Argument example: TemplatePop Surface 3, which will add configuration for 
                        //  three surfaces to the populate section.
                        case "Template":
                            bool success;
                            string templateOutcome = writeTemplate(argReader, _sb, out success);
                            /*
                            //Determine which template we're going to put in populate.
                            string type = argReader.Argument(1).ToLowerInvariant();
                            string subtype = argReader.Argument(2)?.ToLowerInvariant();
                            //Many of these will have an entry for ACTION SECTIONs. And, honestly, 
                            //who can blame them?
                            string actionSection =
                                ";  Any ActionSets this block is tied to. Action Sets are configured from their\n" +
                                ";  own section, the template name of which is 'ActionSection'.\n" +
                                $"ActionSets = < Comma sperated list of ActionSet names, default empty >\n\n";
                            //Surface and MFD types basically share the same template
                            if (type == "surface" || type == "mfdsection")
                            {
                                //The main difference between these two types is what prefix 
                                //they use for their keys.
                                string prefix = type == "surface" ? "Surface" : "Page";
                                //Both will use the fourth argument to figure out which page/
                                //surface number they should put in the key.
                                int num;
                                Int32.TryParse(argReader.Argument(3), out num);
                                //Even subtypes use a lot of the same keys. We'll go ahead and
                                //define them now, so we can re-use them.
                                string fontSizeSection =
                                    ";  The font size to be used. The value must be followed by the letter f,\n" +
                                    ";  so that the parser will realize it's supposed to treat the value as a float.\n" +
                                    $"{prefix}{num}FontSize = < Decimal Value, default 1f >\n\n";
                                string fontSection =
                                    ";  The name of the font to be used.\n" +
                                    $"{prefix}{num}Font = < Font Name, default Debug >\n\n";
                                string colorSection =
                                ...
                                string foreColorSection =
                                    ";  The forground color of this surface.\n" +
                                    $"{colorSection}" +
                                    $"{prefix}{num}ForeColor = < RGB value in the format 0,0,0 or basic color name. >\n\n";
                                string backColorSection =
                                    ";  The background color of this surface.\n" +
                                    $"{colorSection}" +
                                    $"{prefix}{num}BackColor = < RGB value in the format 0,0,0 or basic color name. >\n\n";
                                //The main difference is that MFDs need a CustomTag key to re-
                                //direct the output to a discreet section. We'll go ahead and
                                //add that now, if needed. 
                                if (prefix == "Page" && !hasCustomTag)
                                {
                                    _sb.Append(
                                ...
                                switch (subtype)
                                {
                                    case "report":
                                        _sb.Append(
                                            ...
                                            $"{prefix}{num}Elements = < Comma Seperated List of Element Names >\n\n" +
                                            ";  The title of this particular report, which will appear at the top of its\n" +
                                            ";  surface.\n" +
                                            $"{prefix}{num}Title = < Title, default empty >\n\n" +
                                            ";  The number of columns in the table that will display this report's \n" +
                                            ";  information.\n" +
                                            $"{prefix}{num}Columns = < Integer Value, default 1 >\n\n" +
                                            $"{fontSizeSection}" +
                                            $"{fontSection}" +
                                            ";  The forground color of this report. This will only affect the title,\n" +
                                            ";  as each element sets its own color.\n" +
                                            $"{colorSection}" +
                                            $"{prefix}{num}ForeColor = < RGB value in the format 0,0,0 or basic color name. >\n\n" +
                                            $"{backColorSection}\n");
                                        success = true;
                                    ...
                                    case "datatype":
                                        _sb.Append(
                                            ...
                                            $"{prefix}{num}DataType = < DataType Name >\n\n" +
                                            ...
                                            ";  Raycaster type.\n" +
                                            $"{prefix}{num}DataSource = < DataSource Name >\n\n" +
                                            $"{fontSizeSection}" +
                                            $"{fontSection}" +
                                            ";  A rudimentery text wrap can be applied to these reports. It isn't\n" +
                                            ";  particularly effecient, so it is not recommended for use with types\n" +
                                            ...
                                            $"{prefix}{num}CharPerLine = < Integer Value, default empty >\n\n" +
                                            $"{foreColorSection}" +
                                            $"{backColorSection}");
                                        success = true;
                                        break;
                                    default:
                                        log.add($"Received Template command for type {type}, " +
                                            $"but did not recognize the subtype {subtype}. " +
                                            $"Valid subtypes are: Report, MFD, Script, and DataType");
                                        break;
                                }
                                ...
                                if (prefix == "Page")
                                {
                                    ...
                                    _sb.Append(
                                        $"Page{num}LinkActionSetOn = < Name of ActionSet, default empty >\n\n");
                                    _sb.Append(
                                        ";  The name of the ActionSet whose Off state will cause this MFD\n" +
                                        ";  page to be displayed.\n");
                                    _sb.Append(
                                        $"Page{num}LinkActionSetOff = < Name of ActionSet, default empty >\n\n");
                                }
                            }
                            ...
                            {
                                _sb.Append(
                                    ";  The Tallies that this block reports to.\n");
                                _sb.Append(
                                    "Tallies = < Comma sperated list of Tally names >\n\n" +
                                    $"{actionSection}");
                                success = true;
                            }
                            ...
                            {
                                _sb.Append(
                                    ";  The Tally or ActionSet whose state will be used to define the\n" +
                                    ";  color of this light.\n");
                                _sb.Append(
                            ...
                            else if (type == "raycaster")
                            {
                                ...
                                _sb.Append(
                                    "RaycasterDislayName = < Name to be displayed, default empty >\n\n");
                                _sb.Append(
                            ...
                            {
                                int num;
                                Int32.TryParse(argReader.Argument(2), out num);
                                _sb.Append(
                                    ";  For Tallies, the custom tag 'Me' tells the parser that this\n" +
                                    ";  template will be sent to the Programmable Block's CustomData,\n" +
                                    ";  instead of the Populate group on the grid. This does not need\n" +
                                    ";  to be modified.\n");
                                _sb.Append(
                                    "CustomTag = Me\n\n");
                                _sb.Append(
                                ...
                                _sb.Append(
                                    $"Tally{num}Name = < Name of tally >\n\n");
                                _sb.Append(
                                    ";  The name this tally will display when shown on reports.\n");
                                _sb.Append(
                                    $"Tally{num}DisplayName = < Display Name of tally, default empty >\n\n");
                                _sb.Append(
                                    ";  The type of this tally. Valid tally types are: Inventory, Item,\n" +
                                    ";  Battery, Gas, JumpCharge, Raycast, and PowerProducer.\n");
                                _sb.Append(
                                    $"Tally{num}Type = < Type of tally >\n\n");
                                _sb.Append(
                                ...
                                _sb.Append(
                                    $"Tally{num}ItemTypeID = < Name of item category >\n\n");
                                _sb.Append(
                                    ...
                                    ";  https://github.com/malware-dev/MDK-SE/wiki/Type-Definition-Listing \n");
                                _sb.Append(
                                    $"Tally{num}ItemSubTypeID = < Name of item sub type >\n\n");
                                _sb.Append(
                                ...
                                _sb.Append(
                                    $"Tally{num}Max = < Integer value, default empty >\n\n");
                                _sb.Append(
                                ...
                                _sb.Append(
                                    $"Tally{num}Multiplier = < Integer value, default empty >\n\n");
                                _sb.Append(
                                    ";  Will this element be color-coded using the assumption that low\n" +
                                    ";  values are a good thing?\n");
                                _sb.Append(
                                    $"Tally{num}LowGood = < True/False, default false except for inventory >\n\n");
                                success = true;
                            }
                            ...
                            else if (type == "actionset")
                            {
                                ...
                                Int32.TryParse(argReader.Argument(2), out num);
                                _sb.Append(
                                    ";  For ActionSets, the custom tag 'Me' tells the parser that this\n" +
                                    ";  template will be sent to the Programmable Block's CustomData,\n" +
                                    ";  instead of the Populate group on the grid. This does not need\n" +
                                    ";  to be modified.\n");
                                _sb.Append(
                                ...
                                _sb.Append(
                                    $"Action{num}Name = < Name of ActionSet >\n\n");
                                _sb.Append(
                                    ";  The name this ActionSet will display when shown on reports.\n");
                                _sb.Append(
                                    $"Action{num}DisplayName = < Display Name of ActionSet, default empty >\n\n");
                                _sb.Append(
                                ...
                                _sb.Append(
                                    $"Action{num}ColorOn = < RGB value 0,0,0 or basic color name. Default green. >\n\n");
                                _sb.Append(
                                    ";  The color that will be displayed by linked elements and\n" +
                                    ";  indicators when this ActionSet is 'off'.\n");
                                _sb.Append(
                                    $"Action{num}ColorOff = < RGB value 0,0,0 or basic color name. Default red. >\n\n");
                                _sb.Append(
                                    ";  The text that will be used to describe the ActionSet's state\n" +
                                    ";  on surface elements when the set is 'on'.\n");
                                _sb.Append(
                                    $"Action{num}TextOn = < State descriptor, default 'Enabled' >\n\n");
                                _sb.Append(
                                    ";  The text that will be used to describe the ActionSet's state\n" +
                                    ";  on surface elements when the set is 'off'.\n");
                                _sb.Append(
                                    $"Action{num}TextOff = < State descriptor, default 'Disabled' >\n\n");
                                _sb.Append(
                                ...
                                _sb.Append(
                                    $"Action{num}DelayOn = < Integer value, default 0 >\n\n");
                                _sb.Append(
                                    ";  The number of tics to wait before polling blocks for new data\n" +
                                    ";  when this ActionSet is 'off'.\n");
                                _sb.Append(
                                    $"Action{num}DelayOff = < Integer value, default 0 >\n\n");
                                success = true;
                            }
                            else
                            { 
                                log.add($"Received Template command for unrecognized type " +
                                    $"{type}. Valid types are Subject, Surface, Indicator, " +
                                    $"Raycaster, MFDSection, ActionSection, Tally, and ActionSet.");
                            }
                            */
                        ...
                        break;
                            ...
                            if (argReader.ArgumentCount >= 2 && argReader.ArgumentCount <= 4)
                            {
                            iniReadWrite.Clear();
                            iniReadWrite.TryParse(Me.CustomData);

                            //Split the PB's custom data on an open bracket, which indicates 
                            //the beginning of a section.
                            string[] pbConfig = Me.CustomData.Split('[');
                            int populateIndex = -1;
                            //Figure out which one of these contains the Populate section.
                            for (int i = 0; i < pbConfig.Length; i++)
                            {
                                    if (pbConfig[i].Contains("Populate]"))
                                    {
                                    }
                            }
                            string populateOriginalText = pbConfig[populateIndex];
                            bool success = false;
                            if (!argReader.Switch("add"))
                            {*/
                                //TODO: Monitor. I should go back to using this if it gets fixed
                                /*iniReadWrite.DeleteSection("Populate");*/
                                /*List<MyIniKey> deleteKeys = new List<MyIniKey>();
                                redneckDeleteSection("Populate", iniReadWrite, deleteKeys);
                            }
                            //Determine which template we're going to put in populate.
                            string type = argReader.Argument(1).ToLowerInvariant();
                            string subtype = argReader.Argument(2)?.ToLowerInvariant();
                            //Many of these will have an entry for ActionSections. And, honestly, 
                            //who can blame them.
                            string actionSection =
                                ";  Any ActionSets this block is tied to. Action Sets are configured from their\n" +
                                ";  own section, the template name of which is 'ActionSection'.\n" +
                                $"ActionSets = < Comma sperated list of ActionSet names >\n\n";
                            //Surface and MFD types basically share the same template
                            if (type == "surface" || type == "mfdsection")
                            {
                                //The main difference between these two types is what prefix 
                                ...
                                string backColorSection =
                                    ...
                                    $"{prefix}{num}BackColor = < RGB value in the format 0,0,0 or basic color name. >\n\n";
                                //The main difference is that MFDs need a CustomTag key to re-
                                //direct the output to a discree section. We'll go ahead and
                                //add that now, if needed. And we'll use the built in MyIni 
                                //functions, even though that will limit our ability to format.
                                if (prefix == "Page")
                                {
                                    iniReadWrite.Set("Populate", $"CustomTag", "< Name of target object, prefixed with 'SW.' >");
                                    iniReadWrite.SetComment("Populate", $"CustomTag",
                                        ";  The name of the object this discreet section is meant to configure." +
                                        ";  It will need to have the 'SW.' prefix, as in 'SW.Raycaster' .\n");

                                }
                                //The first step of actually writing the rest of the template 
                                //to CustomData is adding the target key.
                                iniReadWrite.Set("Populate", "Target", "Key");
                                //That's it for MyIni. Push the revised config to the PB.
                                Me.CustomData = iniReadWrite.ToString();
                                //The next step depends on what kind of data the user wants to show.
                                switch (subtype)
                                {
                                    case "report":
                                        Me.CustomData = Me.CustomData.Replace("Target=Key",
                                            ";  The names of the Elements (Tallies or ActionSets) whose status will be\n" +
                                            ";  displayed on this Surface. The word 'blank' can be used to indicate\n" +
                                            ";  that a hole should be left before the next element.\n" +
                                            $"Surface{surface}Elements = < Comma Seperated List of Element Names >\n\n" +
                                            ";  The title of this particular report, which will appear at the top of its\n" +
                                            ";  surface.\n" +
                                            $"Surface{surface}Title = < Title, default empty >\n\n" +
                                            ";  The number of columns in the table that will display this report's \n" +
                                            ";  information.\n" +
                                            $"Surface{surface}Columns = < Integer Value, default 1 >\n\n" +
                                            $"{fontSizeSection}" +
                                            $"{fontSection}" +
                                            ";  The forground color of this report. This will only affect the title,\n" +
                                            ";  as each element sets its own color.\n" +
                                            $"{colorSection}" +
                                            $"Surface{surface}ForeColor = < RGB value in the format 0,0,0 or basic color name. >\n\n" +
                                            $"{backColorSection}\n");
                                        success = true;
                                        break;
                                    case "mfd":
                                    ...
                                    case "script":
                                        Me.CustomData = Me.CustomData.Replace("Target=Key",
                                            ...
                                            ";  TSS_FactionIcon, TSS_Gravity, TSS_Velocity, TSS_VendingMachine,\n" +
                                            ";  TSS_Weather, TSS_Jukebox\n" +
                                            $"Surface{surface}Script = < Script Name >\n\n" +
                                            $"{foreColorSection}" +
                                            $"{backColorSection}\n");
                                        success = true;
                                        break;
                                    case "datatype":
                                        Me.CustomData = Me.CustomData.Replace("Target=Key",
                                            ";  The type of data that will be displayed on this surface. Valid\n" +
                                            ";  DataTypes are: Log, CustomData, DetailInfo, and Raycaster.\n" +
                                            $"Surface{surface}DataType = < DataType Name >\n\n" +
                                            ";  For the CustomData, DetailInfo, and Raycaster DataTypes, a DataSource\n" +
                                            ";  must be specified. This will be the name of a block on the grid for\n" +
                                            ";  CustomData and DetailInfo types, and the name of a Raycaster for the\n" +
                                            ";  Raycaster tpye." +
                                            $"Surface{surface}DataSource = < DataSource Name >\n\n" +
                                            $"{fontSizeSection}" +
                                            $"{fontSection}" +
                                            ";  A rudimentery text wrap can be applied to these reports. It isn't\n" +
                                            ";  particularly effecient, so it is not recommended for use with types\n" +
                                            ...
                                            ";  Note that this value is not a hard limit. The text will be sent to\n" +
                                            ";  the next line only once the length of a word has exceeded this value.\n" +
                                            $"Surface{surface}CharPerLine = < Integer Value >\n\n" +
                                            $"{foreColorSection}" +
                                            $"{backColorSection}\n");
                                        success = true;
                                        break;
                                ...
                                switch (type)
                                {
                                    //Format: TemplatePop Surface Type #
                                    case "surface":
                                        //For Surfaces, the number after the subtype tells us which
                                        //surface this template should target.
                                        int surface;
                                        Int32.TryParse(argReader.Argument(3), out surface);

                                        //A lot of these share common elements. We'll go ahead and 
                                        //define them here, and then use them as nessecary.
                                        string fontSizeSection =
                                            ";  The font size to be used. The value must be followed by the letter f,\n" +
                                            ";  so that the parser will realize it's supposed to treat the value as a float.\n" +
                                            $"Surface{surface}FontSize = < Decimal Value, default 1f >\n\n";
                                        string fontSection =
                                            ";  The name of the font to be used in this report.\n" +
                                            $"Surface{surface}Font = < Font Name, default Debug >\n\n";
                                        string colorSection =
                                            ";  Valid color names are green, lightblue, yellow, orange, and red.\n" +
                                            ";  By default, the color that has been configured for this surface in\n" +
                                            ";  the terminal will be used.\n";
                                        ...
                                        string backColorSection =
                                            ";  The background color of this surface.\n" +
                                            $"{colorSection}" +
                                            $"Surface{surface}BackColor = < RGB value in the format 0,0,0 or basic color name. >\n\n";

                                        //Set our target key.
                                        iniReadWrite.Set("Populate", "Target", "Key");
                                        //That's it for MyIni. Push the revised config to the PB.
                                        Me.CustomData = iniReadWrite.ToString();
                                        //There are several types of reports that can be displayed 
                                        //on a surface, each with a slightly different set of 
                                        ...
                                        break;
                                    //Format: TemplatePop Subject
                                    case "subject":
                                        Me.CustomData = Me.CustomData.Replace("Target=Key",
                                            ";  The Tallies that this block reports to.\n" +
                                            $"Elements = < Comma sperated list of Tally names >\n\n" +
                                            $"{actionSection}\n");
                                        break;
                                    //Format: TemplatePop Indicator
                                    case "indicator":
                                        Me.CustomData = Me.CustomData.Replace("Target=Key",
                                            ";  The tally that will be used to define the color of this light.\n" +
                                            $"Element = < Tally Name >\n\n" +
                                            $"{actionSection}\n");
                                        break;
                                    //Format: TemplatePop Raycaster
                                    case "raycaster":
                                        Me.CustomData = Me.CustomData.Replace("Target=Key",
                                            ";  The name that will be associated with the Raycaster object that\n" +
                                            ";  will be used to perform raycasts with this camera.\n" +
                                            ...
                                            ";  The name that will be used when displaying the charge of this.\n" +
                                            ";  Raycaster in an Element.\n" +
                                            $"RaycasterDislayName = < Name to be displayed >\n\n" +
                                            ";  The maximum distance of the first scan that will be performed\n" +
                                            ";  by this Raycaster, in meters.\n" +
                                            $"RaycasterBaseRange = < Decimal value, default 1000 >\n\n" +
                                            ";  Multiplier that will be applied to the maximum distance of each\n" +
                                            ...
                                            $"{actionSection}\n");
                                        break;
                                    //Format: TemplatePop MFDSection Type #
                                    case "mfdsection":
                                        //For MFDs and ActionSections, the third argument (subtype) 
                                        //will hold the name of the object we're configuring.

                                        //For MFDs, the number after the subtype tells us which page
                                        //this template should target.
                                        int page;
                                        Int32.TryParse(argReader.Argument(3), out page);
                                        Me.CustomData = Me.CustomData.Replace("Target=Key",
                                            $"CustomTag = SW.{subtype}" +
                                            ";  MFDs can display anything a regular report can. Choose one of\n" +
                                            ";  the following keys and un-comment it: \n" +
                                            $";Page{page}Elements = < Comma Seperated List of Element Names >\n" +
                                            $";Page{page}Script = < Script Name >\n" +
                                            $"" +
                                            ";  The name that will be associated with the Raycaster object that\n" +
                                            ";  will be used to perform raycasts with this camera.\n" +
                                            $"RaycasterName = < Name of this Raycaster >\n\n" +
                                            $"" +
                                            $"Page{page}LinkActionSetOn = < Name of linked ActionSet >\n\n");
                                        break;
                                    //Format: TemplatePop ActionSection
                                    case "actionsection":
                                        break;
                                    default:
                                        log.add($"Received Template command for unrecognized type " +
                                            $"{type}. Valid types are Subject, Surface, Indicator, " +
                                            $"Raycaster, MFDSection, and ActionSection");
                                        break;
                                }
                                if (success)
                                {
                                    //Me.CustomData = iniReadWrite.ToString();
                                    log.add($"Added {type} template to the Populate section of this " +
                                        "Programmable Block.");
                                }
                                iniReadWrite.Clear();*/
                                /*
                                ...
                                {
                                    iniReadWrite.Set("Populate", $"Surface{i}Elements", "<Comma Seperated List of Element Names>");
                                    iniReadWrite.SetComment("Populate", $"Surface{i}Elements",
                                        "This key defines the type of report that will " +
                                        "appear on this surface. Only one of these keys " +
                                        "should be present for each surface.\n" +
                                        "The names of the Elements (Tallies or ActionSets) " +
                                        "whose status will be displayed on this Surface. The " +
                                        "word 'blank' can be used to indicate that a hole " +
                                        "should be left before the next element.");
                                    iniReadWrite.Set("Populate", $"Surface{i}MFD", "<MFD Name>");
                                    iniReadWrite.SetComment("Populate", $"Surface{i}MFD",
                                        "This key defines the type of report that will " +
                                        "appear on this surface. Only one of these keys " +
                                        "should be present for each surface.\n" +
                                        "The name of the Multi-Function Display that will " +
                                        "be displayed on this surface. MFDs are configured " +
                                        "from their own section, the template name of which " +
                                        "is 'MFDSection'.");
                                    iniReadWrite.Set("Populate", $"Surface{i}Script", "<Script Name>");
                                    iniReadWrite.SetComment("Populate", $"Surface{i}Script",
                                        "This key defines the type of report that will " +
                                        "appear on this surface. Only one of these keys " +
                                        "should be present for each surface.\n" +
                                        "The name of the Script that will be displayed on this " +
                                        "surface. Used primarily on MFD pages.");
                                    iniReadWrite.Set("Populate", $"Surface{i}DataType", "<DataType Name>");
                                    iniReadWrite.SetComment("Populate", $"Surface{i}DataType",
                                        "This key defines the type of report that will " +
                                        "appear on this surface. Only one of these keys " +
                                        "should be present for each surface.\n" +
                                        "The type of data that will be displayed on this " +
                                        "surface. Valid DataTypes are: Log, CustomData, " +
                                        "DetailInfo, and Raycaster.");
                                    iniReadWrite.Set("Populate", $"Surface{i}DataSource", "<DataSource Name>");
                                    iniReadWrite.SetComment("Populate", $"Surface{i}DataSource",
                                        "The source of data for this report. This is " +
                                        "the name of a block for DetailInfo or CustomData " +
                                        "types, and the name of a Raycaster for the Raycaster type.\n" +
                                        "Valid reports: DataType.");
                                    iniReadWrite.Set("Populate", $"Surface{i}Title", "<Title, default empty>");
                                    iniReadWrite.SetComment("Populate", $"Surface{i}Title",
                                        "The title of this particular report, which will " +
                                        "appear at the top of its surface.\n" +
                                        "Valid Reports: ");
                                    iniReadWrite.Set("Populate", $"Surface{i}Columns", "<Integer Value, default 1>");
                                    iniReadWrite.SetComment("Populate", $"Surface{i}Columns",
                                        "The number of columns in the grid that will display " +
                                        "this report's information.");
                                    iniReadWrite.Set("Populate", $"Surface{i}FontSize", "<Decimal Value, default 1f>");
                                    iniReadWrite.SetComment("Populate", $"Surface{i}FontSize",
                                        "The font size to be used. The value must be followed " +
                                        "by the letter f, so that the parser will realize it's " +
                                        "supposed to treat it as a float.");
                                    iniReadWrite.Set("Populate", $"Surface{i}Font", "<Font Name, default Debug>");
                                    iniReadWrite.SetComment("Populate", $"Surface{i}Font",
                                        "The name of the font to be used in this report.");
                                    iniReadWrite.Set("Populate", $"Surface{i}ForeColor", "<RGB value or basic color name.>");
                                    iniReadWrite.SetComment("Populate", $"Surface{i}ForeColor",
                                        "The names of the Elements (Tallies or ActionSets) " +
                                        "whose status will be displayed on this Surface. The " +
                                        "word 'blank' can be used to indicate that a hole " +
                                        "should be left before the next element.");
                            ...
                            //If the user did not give us an acceptable number of arguments
                            /*else
                            { log.add($"Received TemplatePop command with an incorrect number of arguments.");
                            }
                            break;*/

                        //Search the grid for block types compatible withour tallies, and automatically
                        ...
                        case "AutoPopulate":
                            ...
                            //If we find no blocks with configuration, or the 'ignore' flag is set.
                            if (apBlocks.Count == 0 || argReader.Switch("ignore"))
                            {
                                //We'll likely be adding mutiple tallies. This'll help us keep track
                                //of which one we're on.
                                int tallyCounter = 0;
                                string pbConfig = $"[{tag}]";
                                //Batteries
                                GridTerminalSystem.GetBlocksOfType<IMyBatteryBlock>(apBlocks, b =>
                                    b.IsSameConstructAs(Me));
                                if (apBlocks.Count > 0) //Really, what are the odds of not having a battery?
                                {
                                    pbConfig += "\n" +
                                        $"Tally{tallyCounter}Name = Power\n" +
                                        $"Tally{tallyCounter}Type = Battery\n";
                                    tallyCounter++;
                                    foreach (IMyTerminalBlock block in apBlocks)
                                    {
                                        block.CustomData = $"[{tag}]\nTallies = Power";
                                        blockCounter++;
                                    }
                                }
                                //Cargo. We ignore blocks that have the CargoContainer type, but
                                //can't be conveyor'd.
                                GridTerminalSystem.GetBlocksOfType<IMyTerminalBlock>(apBlocks, 
                                ...
                                {
                                    pbConfig += "\n" +
                                        $"Tally{tallyCounter}Name = Cargo\n" +
                                        $"Tally{tallyCounter}Type = Inventory\n" +
                                        $"Tally{tallyCounter}LowGood = true\n";
                                    tallyCounter++;
                                    foreach (IMyTerminalBlock block in apBlocks)
                                    {
                                        block.CustomData = $"[{tag}]\nTallies = Cargo";
                                ...
                                //Hydrogen. Because all GasTanks share the same interface, we need
                                //to make sure our prospective tanks are actually intended to hold
                                //hydrogen.
                                GridTerminalSystem.GetBlocksOfType<IMyGasTank>(apBlocks,
                                    b => b.IsSameConstructAs(Me)
                                ...
                                {
                                    pbConfig += "\n" +
                                        $"Tally{tallyCounter}Name = Hydrogen\n" +
                                        $"Tally{tallyCounter}Type = Gas\n";
                                    tallyCounter++;
                                    foreach (IMyTerminalBlock block in apBlocks)
                                    {
                                        block.CustomData = $"[{tag}]\nTallies = Hydrogen";
                                ...
                                //Oxygen. Because all gas tanks share the same TypeID, and large 
                                //grid oxygen tanks apparently don't have a SubTypeId, we define 
                                //oxygen tanks as "The Gas Tanks that aren't Hydrogen Tanks".
                                GridTerminalSystem.GetBlocksOfType<IMyGasTank>(apBlocks,
                                    b => b.IsSameConstructAs(Me)
                                    && b.BlockDefinition.SubtypeId != "LargeHydrogenTank"
                                    && b.BlockDefinition.SubtypeId != "LargeHydrogenTankSmall"
                                ...
                                {
                                    pbConfig += "\n" +
                                        $"Tally{tallyCounter}Name = Oxygen\n" +
                                        $"Tally{tallyCounter}Type = Gas\n";
                                    tallyCounter++;
                                    foreach (IMyTerminalBlock block in apBlocks)
                                    {
                                ...
                                //Commit the new config to the programmable block
                                Me.CustomData = pbConfig;
                                //Log the results
                                log.add($"Carried out AutoPopulate command, adding {tallyCounter} " +
                                    $"tallies and configuration on {blockCounter} blocks.");
                            ...
                            else
                            {
                                ...
                                _sb.Append("Received AutoPopulate command, but there is already ShipWare " +
                                    "configuration on the grid. Run this command again with the -ignore " +
                                    "switch (Which will overwrite existing configuration where needed), or " +
                                    "use the Clear command to get rid of the configuration on the following " +
                                    "blocks:\n");
                                foreach (IMyTerminalBlock block in apBlocks)
                                {
                                    _sb.Append($" -{block.CustomName}\n");
                                    blockCounter++;
                                    //Let's not spam the log. If there's more than 15 blocks, the 
        ...
        /*
        public int populateSimple(List<IMyTerminalBlock> blocks, MyIni popReference, MyIni targetWriter,
            StringBuilder _sb)
        {
            //If a 'CustomTag' key is present in the PB's Populate section, we'll use that as the
            //section name we're writing configuration to. Otherwise, we'll just use the default 
            ...
            popReference.Delete("Populate", "CustomTag");
            List<MyIniKey> popKeys = new List<MyIniKey>();
            //WORKAROUND: For the misbehaving DeleteSection.
            List<MyIniKey> deleteKeys = new List<MyIniKey>();
            //Get all the keys in the Populate section on the PB
            popReference.GetKeys("Populate", popKeys);
            _sb.Clear();
            ...
            foreach (IMyTerminalBlock block in blocks)
            {
                ...
                //The default version of Populate isn't subtle. We start by simply lopping off any 
                //bits that vaguely resemble the section name we've been given.
                //targetWriter.DeleteSection(populateTag);
                redneckDeleteSection(populateTag, targetWriter, deleteKeys);
                //We want to use String.Replace to put our newSection in. Before we do that, we need
                //to make sure there's a section for us to target.
                targetWriter.AddSection(populateTag);
                //Make the initial edit to the CustomData, which should contain the section heading
                block.CustomData = targetWriter.ToString();
                //Replace that heading with the newSection we built earlier.
                block.CustomData = block.CustomData.Replace($"[{populateTag}]", newSection);
            }
            //Be a good neighbor and clear out the globals we used.
            popReference.Clear();
            targetWriter.Clear();
        ...
        public string populatePB(string source, string section, string type, int ID)
        {
            string targetPrefix = $"{type}{ID}";
            string endPrefix = $"{type}{ID + 1}";
            int lineCounter = 0;
            int targetIndex = -1;
            bool foundSection = false;
            List<string> sourceByLine = source.Split('\n').ToList();
            foreach (string line in sourceByLine)
            {
                //If we just found the section we're looking for
                if (!foundSection && line.Contains($"[{section}]"))
                {
                    foundSection = true;
                    targetIndex = lineCounter;
                }
                else if (foundSection)
                {
                    if (line.Contains(targetPrefix) ||)
                    //Every line with the target prefix after the first gets removed.
                    { sourceByLine.RemoveAt(lineCounter); }
        ...
        //The original, single-mode Populate
        /*public int populate(popMode mode, List<IMyTerminalBlock> blocks, MyIni popReference, MyIni targetWriter)
        {
            int editCounter = 0;
            //If a 'CustomTag' key is present in the PB's Populate section, we'll use that as the
            //section name we're writing configuration to. Otherwise, we'll just use the default 
            ...
            foreach (IMyTerminalBlock block in blocks)
            {
                ...
                /*targetWriter.DeleteSection(populateTag);
                /*WORKAROUND
                targetWriter.GetKeys(populateTag, deleteKeys);
                foreach (MyIniKey key in deleteKeys)
                { targetWriter.Delete(key); }
                /*END WORKAROUND 
                //For every key in the Populate section on the PB...
                foreach (MyIniKey key in popKeys)
                {
                    //Write the contents of the key in the Populate section to the configuration 
                    //we're building in the targetWriter, using the section name we've been given.
                    targetWriter.Set(populateTag, key.Name, popReference.Get(key).ToString());
                    //Increment our tally.
                    editCounter++;
                }
        ...
        //List<string> sections: The list that will hold the remaining sections
        ...
        public string writeTemplate(MyCommandLine argReader, StringBuilder _sb, out bool isSuccessful)
        {
            ...
            if (type == "surface" || type == "mfdsection")
            {
                ...
                for (int i = 2; i < argReader.ArgumentCount; i++)
                {
                    ...
                    if (subtype != "blank")
                    {
                        //Even subtypes use a lot of the same keys. We'll go ahead and
                        //define them now, so we can re-use them.
                        fontSizeKey = $";{prefix}{surface}FontSize = < Decimal Value, default 1f >\n";
                        ...
                        switch (subtype)
                        {
                            case "report":
                                _sb.Append(elementsFlag ? "" :
                                    ...
                                    ";  displayed on this Surface. The word 'blank' can be used to indicate\n" +
                                    ";  that a hole should be left before the next element.\n");
                                _sb.Append($"{prefix}{surface}Elements = < Comma Seperated List of Element Names >\n");
                                elementsFlag = true;
            ...
            else if (type == "tally")
            {
                ...
                if (!Int32.TryParse(argReader.Argument(2), out ID))
                {
                    ...
                    ID = findOpening(tag, "Tally", iniRead);
                    iniRead.Clear();
                }
                _sb.Append(header);
                _sb.Append(
            ...
            else if (type == "actionset")
            {
                ...
                if (!Int32.TryParse(argReader.Argument(2), out ID))
                {
                    iniRead.Clear();
                    iniRead.TryParse(Me.CustomData);
                    ID = findOpening(tag, "Action", iniRead);
                    iniRead.Clear();
                }
                _sb.Append(header);
                _sb.Append(
        ...
        public void initiate()
        {
            ...
            log = new Hammers.EventLog(_sb, "Ship Manager - Recent Events", true);
        ...
        public void evaluate()
        {
            ...
            //  Inventory, Item, Battery, Gas, JumpCharge, Raycast, PowerProducer (Solar/Wind),
            //  HydrogenWithEngines? 
            ...
            while (counter != -1)
            {
                ...
                elementName = iniReadWrite.Get(tag, $"Tally{counter}Name").ToString();
                ...
                else if (!string.IsNullOrEmpty(elementName))
                {
                    ...
                    tallyType = iniReadWrite.Get(tag, $"Tally{counter}Type").ToString();
                    //If no type is defined...
                    if (string.IsNullOrEmpty(tallyType))
                    {
                        //... complain.
                        errors.add($"Tally {elementName} has a missing or unreadable TallyType.");
                        ...
                        tally = new TallyCargo(meterMaid, elementName);
                    }
                    ...
                    else if (tallyType == "Item")
                    {
                        ...
                        addIn1 = iniReadWrite.Get(tag, $"Tally{counter}ItemTypeID").ToString();
                        //If we can't get it, complain.
                        if (string.IsNullOrEmpty(addIn1))
                        { errors.add($"Item Tally '{elementName}' has a missing or unreadable TallyItemTypeID."); }
                        //And a SubTypeID, stored in AddIn2
                        addIn2 = iniReadWrite.Get(tag, $"Tally{counter}ItemSubTypeID").ToString();
                        if (string.IsNullOrEmpty(addIn2))
                        { errors.add($"Item Tally '{elementName}' has a missing or unreadable TallyItemSubTypeID."); }
                        ...
                        { tally = new TallyItem(meterMaid, elementName, addIn1, addIn2); }
                    ...
                    { tally = new TallyGeneric(meterMaid, elementName, handleBattery); }
                    //Gas, which works for both Hydrogen and Oxygen
                    else if (tallyType == "Gas")
                    { tally = new TallyGeneric(meterMaid, elementName, handleGas); }
                    //JumpCharge
                    else if (tallyType == "JumpCharge")
                    { tally = new TallyGeneric(meterMaid, elementName, handleJumpCharge); }
                    //Raycst
                    else if (tallyType == "Raycast")
                    { tally = new TallyGeneric(meterMaid, elementName, handleRaycaster); }
                    //MaxOutput
                    else if (tallyType == "PowerProducer")
                    { tally = new TallyGeneric(meterMaid, elementName, handlePowerProducer); }
                    ...
                    else
                    {
                        ...
                        errors.add($"Tally {elementName}'s TallyType of '{tallyType}' cannot be handled" +
                    ...
                    iniValue = iniReadWrite.Get(tag, $"Tally{counter}DisplayName");
                    if (!iniValue.IsEmpty)
                    { tally.name = iniValue.ToString(); }
                    //Then the Max
                    iniValue = iniReadWrite.Get(tag, $"Tally{counter}Max");
                    if (!iniValue.IsEmpty)
                    ...
                    {
                        errors.add($"Tally {elementName}'s TallyType of '{tallyType}' requires a Max " +
                            $"to be set in configuration.");
                    }
                    //Up next is the Multiplier
                    iniValue = iniReadWrite.Get(tag, $"Tally{counter}Multiplier");
                    if (!iniValue.IsEmpty)
                    { tally.multiplier = iniValue.ToDouble(); }
                    //Last, LowGood
                    iniValue = iniReadWrite.Get(tag, $"Tally{counter}LowGood");
                    if (!iniValue.IsEmpty)
            ...
            while (counter != -1)
            {
                ...
                elementName = iniReadWrite.Get(tag, $"Action{counter}Name").ToString();
                ...
                else if (!string.IsNullOrEmpty(elementName))
                {
                    ...
                    bool state = iniRead.Get("ActionSets", elementName).ToBoolean(false);
                    set = new ActionSet(elementName, state);
                    //There are a few other bits of configuration that ActionSets may have
                    //DisplayName
                    iniValue = iniReadWrite.Get(tag, $"Action{counter}DisplayName");
                    if (!iniValue.IsEmpty)
                    { set.name = iniValue.ToString(); }
                    //OnColor
                    if (tryGetColorFromConfig(errors, ref color, $"Action{counter}", "ColorOn", 
                        tag, iniReadWrite, Me))
                    { set.colorOn = color; }
                    //OffColor
                    if (tryGetColorFromConfig(errors, ref color, $"Action{counter}", "ColorOff",
                        tag, iniReadWrite, Me))
                    { set.colorOff = color; }
                    //OnText
                    iniValue = iniReadWrite.Get(tag, $"Action{counter}TextOn");
                    if (!iniValue.IsEmpty)
                    { set.textOn = iniValue.ToString(); }
                    //OffText
                    iniValue = iniReadWrite.Get(tag, $"Action{counter}TextOff");
                    if (!iniValue.IsEmpty)
                    ...
                    int delayOn = iniReadWrite.Get(tag, $"Action{counter}DelayOn").ToInt32();
                    int delayOff = iniReadWrite.Get(tag, $"Action{counter}DelayOff").ToInt32();
            ...
            if (errors.getErrorTotal() == 0)
            {
                    ...
                    if (iniReadWrite.TryParse(block.CustomData, out parseResult))
                    {
                        ...
                        if (iniReadWrite.ContainsKey(tag, "RaycasterName"))
                        {
                            ...
                            if (!evalTallies.ContainsKey(elementName))
                            {
                                ...
                                tallyGeneric = new TallyGeneric(meterMaid, elementName, handleRaycaster);
                                ...
                                if (!iniValue.IsEmpty)
                                { tallyGeneric.name = iniValue.ToString(); }
                                //BaseRange
                ...
                /*errors.add(Hammers.findBlocks<IMyTerminalBlock>(GTS, blocks, b =>
                    (b.IsSameConstructAs(Me) && MyIni.HasSection(b.CustomData, tag)),
                        ...
                        if (evalTallies.ContainsKey(name))
                        {
                            ...
                            else if (tally is TallyGeneric)
                            {
                                ...
                                if (tallyGeneric.currHandler == handleGas)
                                {
                                    ...
                                    {
                                        errors.add($"Block '{block.CustomName}' is not a gas tank " +
                                ...
                                else if (tallyGeneric.currHandler == handleBattery)
                                {
                                    ...
                                    {
                                        errors.add($"Block '{block.CustomName}' is not a battery " +
                                            $"and is not compatible with the TallyType of " +
                                            $"tally '{name}'.");
                                    }
                                }
                                else if (tallyGeneric.currHandler == handleJumpCharge)
                                {
                                    ...
                                    {
                                        errors.add($"Block '{block.CustomName}' is not a jump drive " +
                                            $"and is not compatible with the TallyType of " +
                                            $"tally '{name}'.");
                                    }
                                }
                                else if (tallyGeneric.currHandler == handlePowerProducer)
                                {
                                    ...
                                    {
                                        errors.add($"Block '{block.CustomName}' is not a power producer " +
                                            $"and is not compatible with the TallyType of " +
                                            $"tally '{name}'.");
                                    }
                                }
                                else if (tallyGeneric.currHandler == handleRaycaster)
                                {
                                    ...
                                    {
                                        errors.add($"Block '{block.CustomName}' is not a camera " +
                                            $"and is not compatible with the TallyType of " +
                                            $"tally '{name}'.");
                                    }
                                }
                                ...
                                //...I done goofed.
                                {
                                    errors.add($"Block '{block.CustomName}' refrenced the tally " +
                        ...
                        if (sets.ContainsKey(name))
                        {
                            ...
                            if (iniReadWrite.ContainsSection(discreetTag))
                            {
                                ...
                                if (actionPlan == null || !actionPlan.hasAction())
                                {
                                    ...
                                    blockPlan.actionOn = tryGetActionFromConfig(errors, discreetTag,
                                        "ActionOn", block, iniReadWrite, actions);
                                    //Check to see if there's an ActionOff in the ACTION SECTION
                                    blockPlan.actionOff = tryGetActionFromConfig(errors, discreetTag,
    ...
    */
        ...
        public Action<IMyTerminalBlock> tryGetActionFromConfig(LimitedErrorLog errors, string discreetTag, string target, 
            IMyTerminalBlock block, MyIni iniReader, Dictionary<string, Action<IMyTerminalBlock>> actions)
        {
            ...
            if (!iniValue.IsEmpty)
            {
                ...
                //If this string matches an entry in our 'actions' dictionary...
        ...
        public class LimitedErrorLog
        {
            ...
            //Add an error to the log, or increment the overflow counter if we've to many errors already.
        ...
        public abstract class Tally : IHasElement
        {
            ...
            { return $"{name, -12}{($"{readableCurr} / {readableMax}"), -12}{meter}"; }
        ...
        public class TallyGeneric : Tally
        {
            ...
            //however.
            public TallyGeneric(Hammers.MeterMaid meterMaid, string name, Func<List<IMyTerminalBlock>, double> handler, 
            ...
            internal override void compute()
            {
                ...
                //Now, the meter.
                meterMaid.drawMeter(ref meter, percent);
        ...
        //sensitive about things, and we need to know how to ask them if they have an action.
        ...
        */
        ...
        public class Hammers
        {
            ...
            public class EventLog
            {
                ...
                //the one that the reset of the script uses.
                StringBuilder _sb;
