...
{
    //ARCHIVE: Created just prior to re-working Populate to work with loadExisting and RawTextIni
    ...
    //Derived from DefenseShield's PB API. Most of this code was written, I assume, by DarkStar.
    //Fairly significant modifications have been made to better work with this script, and some of
    //the delegates have been left out because this script didn't use them.
    //The original API can be found at:
    ...
    {
        //The maximum heat that shields can reach. There's no getter for this, so we make a guess 
        //and write it down
        public const int HEAT_MAX = 100;
        //The reference block that the delegates will go to to get their data.
        public IMyTerminalBlock refBlock { get; private set; }
        //Has this instance of the API successfully retrieved its delegates?
        //(It takes ~10 tics after a shield becomes active in the world for the dictionary to be
        //placed on TerminalBlocks. If there's no dictionary, it may indicate that the mod isn't 
        //loaded)
        public bool delegatesLoaded { get; private set; }
        //Have we confirmed that the reference block is actually a shield block?
        //(If the reference block hasn't been recognized as a shield, most of the delegates will 
        //return null-equivilants. A shield block isn't recognized as such until it's been on at
        //least once in this instance of the world).
        public bool refCertified { get; private set; }

        //Storage for the delegates provided by the DefenseShieldAPI (This isn't all of them, just 
        //the ones we need)
        ...
        public ShieldAPI(IMyTerminalBlock pb, IMyTerminalBlock controller)
        {
            /*
            refBlock = block;
            //Try to get the shield API dictionary from the reference block
            //IMPORTANT: The original code here gets a property called "DefenseSystemsPbAPI", which
            //is unique to the PB. Because we're expecting to have a reference to the Shield 
            //Controller block passed to us, we try to retrieve its version of the delegate dictionary
            //instead. Its name is DefenseSystemsAPI.
            */
            //Try to retrieve the delegate dictionary from the PB. The PB's version of this dictionary 
            //is different from other shield block's 'DefenseSystemsAPI', which is why we need a 
            //reference to the PB alongside the reference to the controller block that we'll use for
            //most of this script.
            Dictionary<string, Delegate> delegates = pb.GetProperty("DefenseSystemsPbAPI")?.As<Dictionary<string, Delegate>>().GetValue(pb);

            if (delegates == null)
            {
                //If we can't find the dictionary, the mod isn't loaded in this world. That may be
                //because the mod itself hasn't been included, or it may mean that it simply hasn't
                //been activated yet (Usually the case after world initialization).
            ...
            //Store the delegates from the API dictionary in this object's variables.
            ...
            /*
            //If the refBlock isn't a shield block, replace it with one that is
            //(This is original code. We're actually going to try to find the shield controller
            //in the main method and pass it in instead of relying on this)
            if (!IsShieldBlock())
            { refBlock = GetShieldBlock(refBlock.CubeGrid) ?? refBlock; }
            */
            //In the original code, a call to GetShieldBlock was made here in order to... get a 
            //shield block, and use that instead of the PB we presumably started with. But we don't
            //need that, because we've explicity been passed a shield controller in the arguments.
            refBlock = controller;
        }

        //Check to see if the refBlock is being recognized as a shield block.
        public bool checkReference()
        {
            refCertified = IsShieldBlock();
            return refCertified;
        }
        
        //The maximum possible health of the shield
        public float GetMaxCharge() => _getMaxCharge?.Invoke(refBlock) ?? -1;
        //The current health of the shield
        public float GetCharge() => _getCharge?.Invoke(refBlock) ?? -1;
        //The current health of the shield against the maximum health of the shield
        public float GetShieldPercent() => _getShieldPercent?.Invoke(refBlock) ?? -1;
        //The current heat level of the shield
        public int GetShieldHeat() => _getShieldHeat?.Invoke(refBlock) ?? -1;
        //The rate at which the shield is currently recharging
        public float GetChargeRate() => _getChargeRate?.Invoke(refBlock) ?? -1;
        //Depreceated? I think you used to be able to exchange some of your charge rate for beefier
        //shields (And vice versa). But I don't see controls for that anymore, and it seems to return
        //100 at all times.
        public float HpToChargeRatio() => _hpToChargeRatio?.Invoke(refBlock) ?? -1;
        public bool IsShieldUp() => _isShieldUp?.Invoke(refBlock) ?? false;
        //A string descriptor of the shield's current status (Up, Down, etc). Returns an empty string
        //if there's no shield on the grid.
        public string ShieldStatus() => _shieldStatus?.Invoke(refBlock) ?? string.Empty;
        //Gets one of the defense shield blocks on the target grid?
        public IMyTerminalBlock GetShieldBlock(IMyEntity entity) => _getShieldBlock?.Invoke(entity) ?? null;
        //Is this block a shield block?
        public bool IsShieldBlock() => _isShieldBlock?.Invoke(refBlock) ?? false;
    }

    /* An attempt to modify the ShieldAPI so that it could re-try retrieval of the DefenseShieldAPI
     * delegates. Abandoned because the delegates are stored in read-only variables that can only
     * be set in the constructor, which I didn't want to mess with.
    public class ShieldAPI
    {
        //The maximum heat that shields can reach. There's no getter for this, so we make a guess 
        //and write it down (It's a pretty good guess)
        public const int HEAT_MAX = 100;
        //The reference block that the delegates will go to to get their data.
        private IMyTerminalBlock refBlock;
        //Does this world even have the DefenseShields mod loaded?
        ...
        private readonly Func<IMyTerminalBlock, float> _getCharge;
        private readonly Func<IMyTerminalBlock, string> _shieldStatus;
        //We don't need these, but the API does.
        private readonly Func<IMyEntity, IMyTerminalBlock> _getShieldBlock;
        private readonly Func<IMyTerminalBlock, bool> _isShieldBlock;

        public ShieldAPI(IMyTerminalBlock block)
        {
            //Set a block that the handler will go to to get the dictionary containing information
            //about the shield. Every block on the grid has a reference to this dictionary, this is
            //simply for conveinience.
            refBlock = block;
            //Try to get the shield API dictionary from the reference block
            Dictionary<string, Delegate> delegates = refBlock.GetProperty("DefenseSystemsPbAPI")?.As<Dictionary<string, Delegate>>().GetValue(refBlock);

            if (delegates == null)
            {
                //If we can't find the dictionary, the mod isn't loaded.
                isLoaded = false;
                return;
            }
            else
            ...
            //Maybe it replaces the reference block (Which could be anything) with a shield block
            //(Emitter, controller, etc) that's also on the grid? But why? And why after everything
            //else has been done?
            if (!IsShieldBlock())
            { refBlock = GetShieldBlock(refBlock.CubeGrid) ?? refBlock; }
        }

        //Attempt to retrieve the DefenseShieldAPI delegates from the stored refBlock. Will also 
        //attempt to set the refBlock to a shield block that's on the grid.
        public bool finishSetup()
        {
            if (retrieveDelegates())
            {
                //I still have no idea what this does. But it looks important.
                //I think it replaces the reference block (Which could be anything) with a shield 
                //block (Emitter, controller, etc) that's also on the grid? But I'm not sure why,
                //when every block on the grid has access to the dictionary.
                if (!IsShieldBlock())
                { refBlock = GetShieldBlock(refBlock.CubeGrid) ?? refBlock; }
            }
        }

        //Attempt to retrieve the DefenseShieldAPI delegates from the stored refBlock.
        private bool retrieveDelegates()
        {
            Dictionary<string, Delegate> delegates = refBlock.GetProperty("DefenseSystemsPbAPI")?.As<Dictionary<string, Delegate>>().GetValue(refBlock);
        ...
        //For debugging use
        public string refBlockByName()
        { return refBlock.CustomName; }

        //The following methods are what actually pull data from the delegates.
        //The maximum possible health of the shield
        public float GetMaxCharge() => _getMaxCharge?.Invoke(refBlock) ?? -1;
        //The current health of the shield
        public float GetCharge() => _getCharge?.Invoke(refBlock) ?? -1;
    ...
    */
    ...
    partial class Program : MyGridProgram
    {
        ...
        //The ShieldBroker object that manages and grants access to the API that interfaces with the 
        //DefenseShields mod.
        ShieldBroker shieldBroker;

        public class ShieldBroker
        {
            //The number of times a call to this broker needs to fail before we decide that the
            //shield mod isn't loaded
            const int MAX_TIMEOUT = 15;

            //The DefenseShield PBWrapper this broker manages
            public ShieldAPI shieldAPI { get; private set; }
            //A reference to the Programmable Block this script is running on
            IMyTerminalBlock pb;
            //A reference to the Controller block we passed in to this object
            IMyTerminalBlock controller;
            //How many more requests for the ShieldAPI will this broker honor? 
            public int currTimeout { get; private set; }
            //A string describing the current status of the broker
            public string status { get; private set; }
            //Has the contained ShieldAPI object successfully retrieved its delegates? [Obsolete?]
            public bool delegatesLoaded
            { get
                { return shieldAPI.delegatesLoaded; }
            }
            //What is that reference block, anyway?
            public IMyTerminalBlock refBlock
            {
                get
                { return shieldAPI.refBlock; }
            }
            //Has the broker deemed the ShieldAPI ready for use?
            public bool apiReady;

            public ShieldBroker(IMyTerminalBlock pb, IMyTerminalBlock controller)
            {
                this.pb = pb;
                this.controller = controller;
                shieldAPI = new ShieldAPI(pb, controller);
                currTimeout = MAX_TIMEOUT;
                status = "<Initiated>";
                apiReady = false;
            }

            //Examine the API to see if it's in a working state. This checks to see if the delegates
            //have been retrieved, and that the refBlock has been certified as a shield block. If 
            //boh of these conditions are met, or if the broker has timed out waiting for the 
            //delegates, this method will return true to indicate that the broker can be retired.
            public bool examineAPI()
            {
                //If we've timed out
                if (currTimeout == -1)
                {
                ...
                {
                    //If the API hasn't succcessfully retrieved the delegates...
                    if (!shieldAPI.delegatesLoaded)
                    {
                        //Attempt to re-initialize the ShieldAPI object
                        shieldAPI = new ShieldAPI(pb, controller);
                        status = "<Retrying>";
                        currTimeout--;
                        return false;
                    }
                    //If we have successfully retrieved the delegates...
                    else
                    {
                        //If the reference block has yet to be certified...
                        if (!shieldAPI.refCertified)
                        {
                            //Re-check the reference block
                            shieldAPI.checkReference();
                            status = "<Certifying>";
                            return false;
                        }
        ...
        public void Main(string argument, UpdateType updateSource)
        {
            ...
            if ((updateSource & UpdateType.Update100) != 0)
            {
                ...
                if (distributor.tic())
                {
                    //...by all means, do the update. Start with checking out the shieldBroker
                    bool retireBroker = shieldBroker?.examineAPI() ?? false;
                    if (retireBroker)
                    //If the broker is no longer needed, null this reference to it so we stop feeding
                    //it tics.
                    { shieldBroker = null; }
                    /* Debug use */
                    else
                    {
                        if (shieldBroker != null)
                        { log.add($"ShieldBroker on timeout tic {shieldBroker.currTimeout}. RefBlock is {shieldBroker.refBlock}."); }
                        //else
                        //{ log.add("ShieldBroker has retired."); }
                    }
                    compute();
                    update();
            ...
            else
            {
                ...
                if (argReader.TryParse(argument))
                {
                        ...
                        case "populate":
                            ...
                            if (iniRead.TryParse(Me.CustomData, out parseResult))
                            {
                                ...
                                else if (iniRead.ContainsSection("Populate"))
                                {
                                    ...
                                    //TODO: Monitor. The last place I used this checked to see if the 
                                    //Block group that I'm not bothering to assign to a variable here
                                    //was null before trying to get the block list from it. Maybe one
                                    //of those question mark things to let it know there may be a null
                                    //coming...
                    ...
                    case "loadexisting":
                        ...
                        GridTerminalSystem.GetBlockGroupWithName("Populate")?.GetBlocks(existingBlocks);
                        if (existingBlocks.Count > 0)
                        {
                            ...
                            if (!iniRaw.tryLoad(iniReadWrite, existingParseResult, targetBlock.CustomData))
                            {
                            ...
                            {
                                string readSection;
                                iniRaw.tryRetrieveSectionContents(sectionName, out readSection);
                                ...
                                iniRaw.tryLoad(iniReadWrite, existingParseResult, Me.CustomData);
                        ...
                        {
                            log.add("Received LoadExisting command, but there is no Populate " +
                                "block group on the grid.");
                        }
                        break;
        ...
        public int populateSimple(List<IMyTerminalBlock> blocks, MyIni popReference, StringBuilder _sb)
        {
            //If a 'CustomTag' key is present in the PB's Populate section, we'll use that as the
            //section name we're writing configuration to. Otherwise, we'll just use the default 
            //script tag
            string populateTag = popReference.Get("Populate", "CustomTag").ToString(tag);
            //We will make one change to the popReference: Now that we have CustomTag, we're going
            //to pretend it isn't there.
            popReference.Delete("Populate", "CustomTag");

            List<MyIniKey> popKeys = new List<MyIniKey>();
            //Get all the keys in the Populate section on the PB
            popReference.GetKeys("Populate", popKeys);
            _sb.Clear();
            _sb.Append($"{populateTag}]\n");
            //Build a new section using the list of keys and their contents from the popReference.
            foreach (MyIniKey key in popKeys)
            { _sb.Append($"{key.Name} = {popReference.Get(key).ToString()}\n"); }
            _sb.Append("\n");
            string newSection = _sb.ToString();

            int index = -1;
            List<string> sections = new List<string>();
            //For every block in the Populate group...
            foreach (IMyTerminalBlock block in blocks)
            {
                //The default version of Populate isn't subtle. We start by simply lopping off any 
                //bits that vaguely resemble the section name we've been given.
                index = redneckDeleteSection(block.CustomData, populateTag, ref sections);
                //If there was no section with the populateTag, add a new one at the top.
                if (index == -1)
                {
                    index = 0;
                    sections.Insert(index, "");
                }
                //Write the new config we assembled earlier to the now-empty section
                sections[index] = newSection;

                _sb.Clear();
                //Re-assemble the config, tacking on the open bracket we used as a delineator.
                foreach (string section in sections)
                { _sb.Append($"[{section}"); }
                //Write the new config to the block
                block.CustomData = _sb.ToString();
            }
            //Be a good neighbor and clear out the globals we used.
            popReference.Clear();
            _sb.Clear();
            //Unlike the other method, we don't really get a good count on edits out of this. So we
            //just make one up.
            return blocks.Count * popKeys.Count;
        }
         ...
         *   This will only be used as a reference (Mostly).
         *  -targetWriter: The other global MyIni object, which will be used to modify the CustomData 
         *   of the blocks in the Populate group.
         * Returns: The number of edits we think we've made.
         */
        public int populate(bool isAdd, List<IMyTerminalBlock> blocks, MyIni popReference, MyIni targetWriter)
        {
            int editCounter = 0;
            //If a 'CustomTag' key is present in the PB's Populate section, we'll use that as the
            //section name we're writing configuration to. Otherwise, we'll just use the default 
            //script tag
            string populateTag = popReference.Get("Populate", "CustomTag").ToString(tag);
            //We will make one change to the popReference: Now that we have the data from CustomTag, 
            //we're going to pretend it isn't there.
            popReference.Delete("Populate", "CustomTag");
            List<MyIniKey> popKeys = new List<MyIniKey>();
            //Get all the keys in the Populate section on the PB
            popReference.GetKeys("Populate", popKeys);
            Dictionary<MyIniKey, string[]> splitContents = null;
            if (isAdd)
            {
                splitContents = new Dictionary<MyIniKey, string[]>();
                foreach (MyIniKey key in popKeys)
                //Add a new entry to the dictionary with the IniKey as its key, containing the 
                //contents of the string split on commas.
                { splitContents.Add(key, popReference.Get(key).ToString().Split(',')); }
            }
            //For every block in the Populate group...
            foreach (IMyTerminalBlock block in blocks)
            {
                //Parse this block's custom data.
                targetWriter.TryParse(block.CustomData);
                //For every key in the Populate section on the PB...
                foreach (MyIniKey key in popKeys)
                {
                    //If we're in overwrite mode...
                    if (!isAdd)
                    //Just copy the contents of this key on the popReference to this key on the target
                    { targetWriter.Set(populateTag, key.Name, popReference.Get(key).ToString()); }
                    else
                    {
                        //Store the initial contents of this key
                        string initialContents = targetWriter.Get(populateTag, key.Name).ToString();
                        //Stores the changes we will make to this key.
                        string alteredContents = initialContents;
                        foreach (string element in splitContents[key])
                        {
                            //If this particular element isn't already in the initialContents...
                            if (!initialContents.Contains(element))
                            //...tack it on to the end.
                            { alteredContents += $", {element}"; }
                        }
                        targetWriter.Set(populateTag, key.Name, alteredContents);
                    }
                    //Increment our tally.
                    editCounter++;
                }
                //Replace the contents of the block's CustomData with the configuration we've altered.
                block.CustomData = targetWriter.ToString();
            }
            //We're done with our MyIni objects. Clear them out to reduce the amount of baggage 
            //we're lugging around.
            popReference.Clear();
            targetWriter.Clear();
            return editCounter;
        }
        ...
        //A stand-in until MyIni's DeleteSection is fixed.
        public void redneckDeleteSectionOLD(string sectionName, MyIni iniTarget, List<MyIniKey> deleteKeys)
        {
            iniReadWrite.GetKeys(sectionName, deleteKeys);
            foreach (MyIniKey key in deleteKeys)
            { iniTarget.Delete(key); }
            //DeleteSection can still be trusted with deleting the section heading.
            iniTarget.DeleteSection(sectionName);
        }
        ...
        public string writeTemplate(MyCommandLine argReader, StringBuilder _sb, out bool isSuccessful)
        {
                        ...
                        switch (subtype)
                        {
                            ...
                            case "datatype":
                                ...
                                _sb.Append(charCountFlag ? "" :
                                    ...
                                    ";  particularly efficient, so it is not recommended for use with types\n" +
                                    ";  that are updated frequently (Like DetailInfo).\n" +
            ...
            else if (type == "tally")
            {
                ...
                _sb.Append(
                    ...
                    ";  that automatically calculated value here. Note that Item, \n" +
                    ";  Raycast, and PowerProducer tally types must have this set manually.\n");
                _sb.Append(
        ...
        public string autoPopulate(IMyGridTerminalSystem GTS, ref List<Tally> tallyList, MyIni configWriter)
        {
            ...
            string refineryConfig = $"[{tag}]";
            string assemblerConfig = $"[{tag}]";
            ...
            /*
            //The rest of the composite tallies are only configured if they've been activated.
            if (useOre)
            {
                //We don't actually generate a tally for composites. Instead, we pass in a null so
                //that portion of configureTally will be bypassed. We also specify that we want to
                //target the first inventory in the refinery blocks
                configureTally(configWriter, tallyDic, "", oreValue, null, refineries,
                    ref tallyList, ref tallyCounter, ref replacementCounter, ref blockCounter, 
                    "Inv0Tallies");
            }
            if (useIngot)
            {
                //Ingots can be found both in refineries...
                configureTally(configWriter, tallyDic, "", ingotValue, null, refineries,
                    ref tallyList, ref tallyCounter, ref replacementCounter, ref blockCounter,
                    "Inv1Tallies");
                //... and assemblers
                configureTally(configWriter, tallyDic, "", ingotValue, null, assemblers,
                    ref tallyList, ref tallyCounter, ref replacementCounter, ref blockCounter,
                    "Inv0Tallies");
            }
            if (useComp)
            {
                configureTally(configWriter, tallyDic, "", compValue, null, assemblers,
                    ref tallyList, ref tallyCounter, ref replacementCounter, ref blockCounter,
                    "Inv1Tallies");
            }*/
            //Shields are weird for several reason, but one is that we aren't actually going to be
            //putting config on any shield blocks. Instead, we look for a controller, and add Tallies
            //to the init list if we find one.
            List<IMyUpgradeModule> upgradeModules = new List<IMyUpgradeModule>();
            bool foundController = false;
            GTS.GetBlocksOfType<IMyUpgradeModule>(upgradeModules, b => b.IsSameConstructAs(Me));
            foreach (IMyUpgradeModule module in upgradeModules)
            {
                if (module.BlockDefinition.SubtypeId == "DSControlTable"
                    || module.BlockDefinition.SubtypeId == "DSControlLarge"
                    || module.BlockDefinition.SubtypeId == "DSControlSmall")
                {
            ...
            if (foundController)
            {
                tallyName = "ShieldHealth";
                reportElements.Add(tallyName);
                if (!tallyDic.ContainsKey(tallyName))
                {
                    newTally = new TallyShield(meterMaid, tallyName, new ShieldHealthHandler(shieldBroker));
                    newTally.displayName = "Shield Health";
                    tallyList.Add(newTally);
                    tallyCounter++;
                }
                tallyName = "ShieldHeat";
                reportElements.Add(tallyName);
                if (!tallyDic.ContainsKey(tallyName))
                {
                    newTally = new TallyShield(meterMaid, tallyName, new ShieldHeatHandler(shieldBroker), true);
                    newTally.displayName = "Shield Heat";
                    tallyList.Add(newTally);
                    tallyCounter++;
                }
                tallyName = "ShieldRegen";
                reportElements.Add(tallyName);
                if (!tallyDic.ContainsKey(tallyName))
                {
                    newTally = new TallyShield(meterMaid, tallyName, new ShieldRegenHandler(shieldBroker));
                    newTally.displayName = "Shield Regen";
                    tallyList.Add(newTally);
                    tallyCounter++;
                }
            }
            /*
            //Shields are weird, because we don't actually check to see if we have any blocks. 
            //Instead, we ask the broker if the API has loaded, and then ask if the grid has any
            //shield blocks.
            //Note: The ShieldAPI seem to take about 10 tics to load after world initialization.
            //Shouldn't be a problem with AutoPopulate, though.
            log.add($"isLoaded: {shieldBroker.isLoaded}\n" +
                $"API isNull: {shieldBroker.getAPI() == null}\n" +
                $"ShieldBlock: {shieldBroker.getAPI().GetShieldBlock(Me.CubeGrid)}\n" +
                $"ShieldBlock NotNull: {shieldBroker.getAPI().GetShieldBlock(Me.CubeGrid) != null}\n" +
                $"ShieldStatus: {shieldBroker.getAPI().ShieldStatus()}");
            */
        ...
        public void initiate()
        {
            ...
            log = new Hammers.EventLog(_sb, "ShipWare - Recent Events", true);
        ...
        public Dictionary<string, Action<IMyTerminalBlock>> compileActions()
        {
            ...
            //May need to resort to terminal actions for AutoDeploy on/off
            //Pistons
                ...
                if (parseResult.Success && iniReadWrite.ContainsKey(tag, "Tallies"))
                {
                    ...
                    foreach (string name in elementNames)
                    {
                        ...
                        if (evalTallies.ContainsKey(name))
                        {
                            ...
                            else if (tally is TallyShield)
                            {
                                errors.add($"Block '{block.CustomName}' has configuration for tally " +
                                    $"'{name}', a Shield tally. Shield tallies do not accept blocks.");
                            }
                            else
            ...
            MyIniValue iniValue, LimitedErrorLog errors, out string tallyName)
        {
            ...
            if (!string.IsNullOrEmpty(tallyName))
            {
                ...
                //Tallies that work with the DefenseShields mod
                /* The PBWrapper takes a few tics to load after world initialization. Because of how
                 * we're compensating for that, we can no longer say definitively if the world has 
                 * shields at time of evaluation. So we just create the tallies and let the 
                 * ShieldBroker deal with any complications.
                else if (tallyType == "ShieldHealth")
                {
                    if (shieldBroker.isLoaded)
                    { tally = new TallyShield(meterMaid, tallyName, new ShieldHealthHandler(shieldBroker)); }
                    else
                    {
                        errors.add(errorShieldMissing + tallyName);
                        //As usual, generate a fake tally so evaluation can continue.
                        tally = new TallyCargo(meterMaid, tallyName);
                    }
                }*/
                else if (tallyType == "ShieldHealth")
                {
                    string outcome = evaluateBroker(tallyName);
                    if (outcome == "")
                    { tally = new TallyShield(meterMaid, tallyName, new ShieldHealthHandler(shieldBroker)); }
                    else
                    { errors.add(outcome); }
                }
                else if (tallyType == "ShieldHeat")
                {
                    string outcome = evaluateBroker(tallyName);
                    if (outcome == "")
                    { tally = new TallyShield(meterMaid, tallyName, new ShieldHeatHandler(shieldBroker)); }
                    else
                    { errors.add(outcome); }
                }
                else if (tallyType == "ShieldRegen")
                {
                    string outcome = evaluateBroker(tallyName);
                    if (outcome == "")
                    { tally = new TallyShield(meterMaid, tallyName, new ShieldRegenHandler(shieldBroker)); }
                    else
                    { errors.add(outcome); }
                }
                ...
                //happened, we need to complain.
                    ...
                    (((TallyGeneric)tally).handler is PowerProducerHandler ||
        ...
        //Makes certain we have a working shieldBroker for evaluation.
        public string evaluateBroker(string tallyName)
        {
            if (shieldBroker == null)
            {
                //We'll just have to create a new block list here. It wouldn't be worth it to run a 
                //reference all this way for just this purpose.
                List<IMyUpgradeModule> prospects = new List<IMyUpgradeModule>();
                IMyUpgradeModule controller = null;
                //Search the grid for UpgradeModules
                GridTerminalSystem.GetBlocksOfType<IMyUpgradeModule>(prospects, b => b.IsSameConstructAs(Me));
                //Check to see if any of the blocks we found have a controller's BlockDefinition
                foreach (IMyUpgradeModule prospect in prospects)
                {
                    if (prospect.BlockDefinition.SubtypeId == "DSControlTable"
                        || prospect.BlockDefinition.SubtypeId == "DSControlLarge"
                        || prospect.BlockDefinition.SubtypeId == "DSControlSmall")
                    {
                        controller = prospect;
                        break;
                    }
                ...
                if (controller != null)
                {
                    shieldBroker = new ShieldBroker(Me, controller);
                    return "";
                }
                //If we couldn't find a shield controller, that's a problem. Complain.
                else
                { return $"Found config for Shield Tally '{tallyName}', but no Shield Controller " +
                        $"block found on the grid."; }
            }
            //If shieldBroker wasn't null, everything is hunky-dory.
            else
            { return ""; }
        ...
        public class RawTextIni
        {
            ...
            // the parser object. If an error is detected during 
            ...
            public bool tryLoad(MyIni parser, MyIniParseResult parseResult, string config)
            {
        ...
        public class TallyShield : Tally
        {
            //The handler that interfaces with the API on behalf of this tally.
            internal ShieldHandler handler;

            //TallyShield is built along the same lines as TallyGeneric, so the constructor is 
            //practically identical.
            public TallyShield(Hammers.MeterMaid meterMaid, string name, ShieldHandler handler,
                bool isLow = false, double multiplier = 1) : base(meterMaid, name, isLow, multiplier)
            {
                this.handler = handler;
            }
            ...
            //None of the values in a TallyShield are fixed (Except MaxHeat on a SheildHeatHandler)
            //so we ignore calls to finishSetup.
            internal override void finishSetup()
            { }

            //Derive the components needed to form an element. Unlike the other Tally types, most 
            //of the heavy lifting is done in the contained handler.
            internal override void compute()
            {
                double hCurr, hMax, hRatio;
                string hTrouble;
                handler.getUpdate(out hCurr, out hMax, out hRatio, out hTrouble);
                //Do we got troubles?
                if (hCurr == -1)
                {
                    //If we got troubles, we leave curr, readableCurr, max, and readableMax as is. 
                    //We'll use the ratio provided by the handler to set the percent, the percent
                    //to set the color code, and the trouble string to set the meter.
                    percent = Math.Min(hRatio * 100, 100);
                    statusColor = colorHandler(percent);
                    meter = hTrouble;
                }
                //If there's no trouble, proceed as normal.
                else
                {
                    //Apply the Tally's multiplier to the handler's curr and max
                    curr = hCurr * multiplier;
                    max = hMax * multiplier;
                    //Multiply the handler's ratio by 100 and cap that number at 100.
                    percent = Math.Min(hRatio * 100, 100);
                    //Next, get the color code from our color handler. 
                    statusColor = colorHandler(percent);
                    //Now, the meter.
                    meterMaid.getMeter(ref meter, percent);
                    //Last, convert curr and max into numbers that you don't need to trail a finger
                    //across to read.
                    Hammers.readableInt(ref readableCurr, (int)curr);
                    Hammers.readableInt(ref readableMax, (int)max);
                }
        ...
        {
            List<IMyGasTank> subjects;

            public GasHandler()
            { subjects = new List<IMyGasTank>(); }

            public bool tryAddBlock(IMyTerminalBlock block)
            {
                IMyGasTank prospect = block as IMyGasTank;
                if (prospect == null)
                { return false; }
                else
                {
            ...
            {
                double max = 0;
                foreach (IMyGasTank tank in subjects)
                { max += tank.Capacity; }
                return max;
            }
        ...
        {
            //A reference to the broker that handles the API
            protected ShieldBroker broker;
            //Once the API checks out, we begin dealing with the API directly
            protected ShieldAPI api;

            public ShieldHandler(ShieldBroker broker)
            {
                this.broker = broker;
                api = null;
            }

            protected bool interrogateBroker(out double curr, out double max, out double ratio, out string trouble)
            {
                curr = -1;
                max = -1;
                ratio = 0;
                trouble = broker.status;
                return broker.apiReady;
            }

            //Pull data from this handler. 
            //  curr: The current value specified by this handler. Returns -1 if trouble is detected
            //  max: The maximum value specified by this handler. Returns -1 if trouble is detected
            //  ratio: The percentage of the current against the max. If trouble is detected, 
            //    returns 1 if the trouble is 'good', 0 if the trouble is bad.
            //  trouble: A string describing any trouble the handler has encountered. If there is no 
            //    trouble, returns an empty string. If trouble is found, returns an error descriptor,
            //    formatted to be used in place of a tally's meter.
            public abstract void getUpdate(out double curr, out double max, out double ratio, out string trouble);

            //Calculate the maximum of this handler
            public abstract double getMax();

            //Calculate the current of this handler
            public abstract double getCurr();

            //Get this hander's config string
            public abstract string writeConfig();
        }

        public class ShieldHealthHandler : ShieldHandler
        {
            public ShieldHealthHandler(ShieldBroker broker) : base(broker)
            { }

            public override void getUpdate(out double curr, out double max, out double ratio, out string trouble)
            {
                if (api == null)
                {
                    //Check to see if the API is ready
                    if (interrogateBroker(out curr, out max, out ratio, out trouble))
                    //If it is, update the local api reference
                    { api = broker.shieldAPI; }
                }
                else
                {
                    curr = getCurr();
            ...
            //The maximum health of the shield, adjusted by the HPToChargeRatio
            public override double getMax()
            { return api.GetMaxCharge() * api.HpToChargeRatio(); }

            //Returns the current charge of the shield adjusted by the HPToChargeRatio (Which may 
            //not do anything anymore)
            public override double getCurr()
            { return api.GetCharge() * api.HpToChargeRatio(); }

            public override string writeConfig()
            { return "ShieldHealth"; }
        }

        public class ShieldHeatHandler : ShieldHandler
        {
            public ShieldHeatHandler(ShieldBroker broker) : base(broker)
            { }
            ...
            public override double getMax()
            { return ShieldAPI.HEAT_MAX; }
            
            public override double getCurr()
            { return api.GetShieldHeat(); }

            public override string writeConfig()
            { return "ShieldHeat"; }
        }

        public class ShieldRegenHandler : ShieldHandler
        {
            //When the shield is fully charged, the API returns a charge rate of 0. This variable 
            //will be used to provide a max in that case.
            private double maxRecordedRegen;

            public ShieldRegenHandler(ShieldBroker broker) : base(broker)
            { maxRecordedRegen = 0; }
            ...
            public override void getUpdate(out double curr, out double max, out double ratio, out string trouble)
            {
                ...
                //Handle the API's handling of full shields
                else if (api.GetChargeRate() <= 0)
                {
                    curr = maxRecordedRegen;
                    max = maxRecordedRegen;
                    ratio = 1;
                    trouble = "";
                }
                else
                {
                    curr = getCurr();
                    if (curr > maxRecordedRegen)
                    { maxRecordedRegen = curr; }
                    max = getMax();
                    ratio = curr / max;
                    trouble = "";
                }
            ...
            //The maximum regeneration rate of the shield's HP.
            //Unfortunately, this value isn't provided by the API, so we'll try to reverse-engineer it.
            //In theory, this formula would be used to adjust the maximum charge rate by the heat level:
            //CurrChargeRate = MaxChargeRate * ((MAX_HEAT - CurrHeat) / MAX_HEAT)
            //So deriving the MaxChargeRate from CurrChargeRate would use this formula
            //MaxChargeRate = CurrChargeRate / ((MAX_HEAT - CurrHeat) / MAX_HEAT)
            //TODO: Replace this, it is not accurate.
            public override double getMax()
            {
                return api.GetChargeRate() / 
                    (((double)ShieldAPI.HEAT_MAX - api.GetShieldHeat()) / ShieldAPI.HEAT_MAX);
            }

            //The current regeneration rate of the shield's HP, adjusted by the HP to Charge ratio. 
            public override double getCurr()
            { return api.GetChargeRate() * api.HpToChargeRatio(); }
