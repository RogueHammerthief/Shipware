...
{
    //ARCHIVE: Created just prior to minifying the script for the first time.
    ...
    partial class Program : MyGridProgram
    {
        ...
        //communication.
        string tag;
        //An enumerator, used to tell what mode to run the Populate command in.
        public enum popMode {reg, add, ow };
                        ...
                        case "Populate":
                            ...
                            if (iniRead.TryParse(Me.CustomData, out parseResult))
                            {
                                ...
                                if (iniRead.ContainsSection("Populate"))
                                {
                                    ...
                                    //coming...
                                    GridTerminalSystem.GetBlockGroupWithName("Populate")?.GetBlocks(blocks);
                                    if (blocks.Count > 0)
                                    {
                                        int edits = 0;
                                        ...
                                        { edits = populate(true, blocks, iniRead, iniReadWrite); }
                                        else if (argReader.Switch("ow"))
                                        { edits = populate(false, blocks, iniRead, iniReadWrite); }
                                        else
                                        //{ edits = populate(popMode.reg, blocks, iniRead, iniReadWrite); }
                                        { edits = populateSimple(blocks, iniRead, iniReadWrite, _sb); }
                            ...
                            break;
                        //Testing implementation of what I hope to do later with the Clear command. 
                        //This one simply clears the custom data of all of the blocks in the populate
                        //group
                        case "Clear":
                            ...
                            { block.CustomData = ""; }
                            log.add($"Clear command executed on {clearBlocks.Count} blocks.");
                            break;
            ...
            StringBuilder _sb)
        {
            ...
            foreach (IMyTerminalBlock block in blocks)
            {
                ...
                /*END WORKAROUND */
                //AddSection is great, because it won't carry through if a section is already present.
                //Unfortunately, sections with different casings on the same characters count as 
                //'already present', and the String.Replace we use later is case sensitive. So we'll
                //call DeleteSection (It can be trusted with this) to make sure we have a clean slate.
                targetWriter.DeleteSection(populateTag);
        ...
        public void initiate()
        {
            ...
            ID = iniReadWrite.Get("Config", "ID").ToString(DEFAULT_ID);
        ...
        public void evaluate()
        {
            ...
            while (counter != -1)
            {
                ...
                else if (!string.IsNullOrEmpty(elementName))
                {
                    ...
                    set.setState(set.state); //Peak readable code.
                ...
                foreach (IMyTerminalBlock block in blocks)
                {
                    ...
                    if (iniReadWrite.TryParse(block.CustomData, out parseResult))
                    {
                        ...
                        if (iniReadWrite.ContainsKey(tag, "RaycasterName"))
                        {
                            ...
                            {
                                errors += $" -The Raycaster name '{elementName}' is already in use " +
                        ...
                        if (sets.ContainsKey(name))
                        {
                            ...
                            if (iniReadWrite.ContainsSection(discreetTag))
                            {
                                ...
                                if (actionPlan == null || !actionPlan.hasAction())
                                {
                                    ...
                                    blockPlan.actionOn = tryGetActionFromConfig(ref errors, discreetTag,
                                        "ActionOn", block, iniReadWrite, actions);
                                    //Check to see if there's an ActionOff in the ACTION SECTION
                                    blockPlan.actionOff = tryGetActionFromConfig(ref errors, discreetTag,
    ...
    */
        ...
        public Action<IMyTerminalBlock> tryGetActionFromConfig(ref string errors, string discreetTag, string target, 
        ...
        */
        ...
        public class Hammers
        {
            ...
            public class EventLog
            {
                ...
                //entries, but it still shouldn't be changing too oftern. We'll store the last one
