...
{
    //ARCHIVE: Created just prior to adding a config reader for ActionPlanTerminal to evaluation.
    ...
    partial class Program : MyGridProgram
    {
                    ...
                    switch (command)
                    {
                        ...
                        */
                        ...
                        case "Reconstitute":
                            ...
                            {
                                Me.CustomData = reconstitute(Me.CustomData, tallies.ToList(), sets.ToList());
        ...
        public string reconstitute(string pbConfig, List<Tally> reconTallies, 
            List<KeyValuePair<string, ActionSet>> reconActions)
        {
            ...
            reconCounter = 0;
            foreach (KeyValuePair<string, ActionSet> pair in reconActions)
            {
                _sb.Append($"{pair.Value.writeConfig(reconCounter, pair.Key)}\n");
                reconCounter++;
            }
        ...
        public string populatePB(string pbConfig, MyIni configReader, out string outcome)
        {
            ...
            List<Tally> reconTallies;
            List<KeyValuePair<string, ActionSet>> reconActions;
            //configReader is already loaded with the parse of pbConfig
            //configReader.Clear();
            //configReader.TryParse(pbConfig);
            ...
            if (type == "Tally")
            {
                Tally newTally = tryGetTallyFromConfig(configReader, "Populate", ID, iniValue,
                ...
                {
                    //Because Tallies are stored in a simple array, the process for figuring out if
                    //we were just handed a duplicate name is somewhat involved.
                    foreach (Tally tally in tallies)
                    {
                        if (initiatorName == tally.programName)
                        {
                            errors.add($"The Tally name '{initiatorName}' is already in use by " +
                                $"another tally."); 
                            break;
                        }
                    }
                    //With actions, this check is much more straightforward.
                ...
                {
                    reconTallies = tallies.ToList();
                    //If this is supposed to go at the end of the list, just use add. Otherwise,
                    //we'll need a replace.
                    if (ID == reconTallies.Count)
                    { reconTallies.Add(newTally); }
                    else
                    {
                        reconTallies.RemoveAt(ID);
                        reconTallies.Insert(ID, newTally);
                    }
                    reconActions = sets.ToList();
                    newConfig = reconstitute(pbConfig, reconTallies, reconActions);
            ...
            else if (type == "Action")
            {
                ...
                ActionSet newAction = tryGetActionFromConfig(configReader, configReader, "Populate", 
                    ID, iniValue, errors, out initiatorName);
                //We'll need sets in a list form for some of our checks.
                reconActions = sets.ToList();
                ...
                //We need to make a check for duplicates. Unless of course the duplicate is right at
                //the index we've been given, in which case we're fine. 
                else if (!(ID < sets.Count() && initiatorName == reconActions[ID].Key))
                {
                    //Because Tallies are stored in a simple array, the process for figuring out if
                    //we were just handed a duplicate name is somewhat involved.
                    foreach (Tally tally in tallies)
                    {
                        if (initiatorName == tally.programName)
                        {
                            errors.add($"The Action name '{initiatorName}' is already in use by " +
                                $"a tally.");
                            break;
                        }
                    }
                ...
                {
                    reconTallies = tallies.ToList();
                    KeyValuePair<string, ActionSet> newPair = new KeyValuePair<string, ActionSet>
                        (initiatorName, newAction);
                    //If this is supposed to go at the end of the list, just use add. Otherwise,
                    //we'll need a replace.
                    if (ID == reconActions.Count)
                    { reconActions.Add(newPair); }
                    else
                    {
                        reconActions.RemoveAt(ID);
                        reconActions.Insert(ID, newPair);
                    }
                    newConfig = reconstitute(pbConfig, reconTallies, reconActions);
                    outcome = $"Carried out Populate command, adding a new initiator for the {type} " +
                        $"{initiatorName} to the programmable block.";
            ...
            { errors.add("Initiator type must be either 'Tally' or 'Action'."); }

            //If the Tally or Action paths executed correctly, they should've already teturned. If
        ...
        /* The original, half-implemented populatePB.
        ...
        public void evaluate()
        {
            ...
            if (errors.getErrorTotal() == 0)
            {
                ...
                log.add($"Grid evaluation complete. Registered {tallies.Length} tallies, " +
                    ...
                    $"data on {blocks.Count} blocks. Evaluation used " +
        ...
        */
            ...
            MyIniValue iniValue, LimitedErrorLog errors, out string tallyName)
        {
            ...
            if (!string.IsNullOrEmpty(tallyName))
            {
                ...
                { tally = new TallyGeneric(meterMaid, tallyName, handleBattery); }
                //Gas, which works for both Hydrogen and Oxygen
                else if (tallyType == "Gas")
                { tally = new TallyGeneric(meterMaid, tallyName, handleGas); }
                else if (tallyType == "JumpCharge")
                { tally = new TallyGeneric(meterMaid, tallyName, handleJumpCharge); }
                else if (tallyType == "Raycast")
                { tally = new TallyGeneric(meterMaid, tallyName, handleRaycaster); }
                else if (tallyType == "PowerProducer")
                { tally = new TallyGeneric(meterMaid, tallyName, handlePowerProducer); }
                //TODO: Aditionally TallyTypes go here
                else
                {
                    //If we've gotten to this point, the user has given us a type that we can't 
    ...
    */
        ...
        public class TallyGeneric : Tally
        {
            ...
            {   //name type max? displayName multiplier lowgood
                ...
                //Getting the type is somewhat involved.
                if (currHandler == handleBattery)
                { config += $"Tally{index}Type = Battery\n"; }
                else if (currHandler == handleGas)
                { config += $"Tally{index}Type = Gas\n"; }
                else if (currHandler == handleJumpCharge)
                { config += $"Tally{index}Type = JumpCharge\n"; }
                else if (currHandler == handleRaycaster)
                { config += $"Tally{index}Type = Raycast\n"; }
                else if (currHandler == handlePowerProducer)
                { config += $"Tally{index}Type = PowerProducer\n"; }
                //Moving on to Max
                if (maxForced)
        ...
        //Stores two lists of Terminal Actions (As ActionParts) that will be executed when this 
        //ActionPlan changes state.
        public class ActionPlanTerminal : IHasActionPlan
        {
            //The TerminalBlock this ActionPlan will be manipulating
            IMyTerminalBlock subject;
            //The action to be performed on the subject block when the ActionPlan is switched on
            private List<ActionPart> actionsOn { get; set; }
            //The action to be performed on the subject block when the ActionPlan is switched off
            private List<ActionPart> actionsOff { get; set; }

            public ActionPlanTerminal(IMyTerminalBlock subject)
            {
                this.subject = subject;
                actionsOn = new List<ActionPart>();
                actionsOff = new List<ActionPart>();
            }

            public void addPartOn<T>(string propertyID, T value)
            { actionsOn.Add(new ActionPart<T>(propertyID, value)); }

            public void addPartOff<T>(string propertyID, T value)
            { actionsOff.Add(new ActionPart<T>(propertyID, value)); }
            ...
            public void takeAction(bool isOnAction)
            {
                if (isOnAction)
                {
                    foreach (ActionPart part in actionsOn)
                    { part.takeAction(subject); }
                }
                else
                {
                    foreach (ActionPart part in actionsOff)
                    { part.takeAction(subject); }
                }
            }
        ...
        //A bit of heresy that makes a genericly typed ActionPart work.
        public abstract class ActionPart
        {
            public abstract void takeAction(IMyTerminalBlock block);
        }

        //Stores the type, propertyID, and value of an individual ActionPart, and allows SetValue to
        //be run.
        public class ActionPart<T> : ActionPart
        {
            //The property this ActionPart will target
            string propertyID;
            //The value this ActionPart will apply when action is taken.
            private T value;

            public ActionPart(string propertyID, T value)
            {
                this.propertyID = propertyID;
                this.value = value;
            }

            public override void takeAction(IMyTerminalBlock block)
            { block.SetValue<T>(propertyID, value); }
        }
        ...
        public class ActionSet : IHasElement
        {
            ...
            {
                //Becaues we may need to access it later, we want to make sure that ActionPlanUpdates
                //are at the top of the list.
                if (plan is ActionPlanUpdate)
            ...
            public string writeConfig(int index, string programName)
            {
                //Default values for this config. 
                ...
                //Getting DelayOn and DelayOff is a bit involved.
                //But we've made it a bit easier on ourselves by always putting ActionPlanUpdates 
                //at the top of the list in addActionPlan.
                if (actionPlans.Count > 0 && actionPlans[0] is ActionPlanUpdate)
                {
                    ActionPlanUpdate updatePlan = (ActionPlanUpdate)actionPlans[0];
