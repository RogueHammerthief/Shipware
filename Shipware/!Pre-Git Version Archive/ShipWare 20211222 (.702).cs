...
namespace IngameScript
{
    ...
    partial class Program : MyGridProgram
    {
        ...
        public void Save()
        {
            ...
            iniReadWrite.Set("Data", "Version", VERSION);
            //Store the ID of this instance of the script as well.
            iniReadWrite.Set("Data", "ID", customID);
            //Store the current value of the update delay
            iniReadWrite.Set("Data", "UpdateDelay", distributor.updateDelay);
            if (sets != null)
            {
                ...
                { iniReadWrite.Set("ActionSets", setName, sets[setName].isOn); }
            }
            if (triggers != null)
            {
                ...
                { iniReadWrite.Set("Triggers", trigger.programName, trigger.enabled); }
            }
            if (MFDs != null)
            {
                    ...
                    switch (command)
                    {
                        ...
                        */
                        ...
                        case "action":
                            ...
                            if (argReader.ArgumentCount == 3)
                            {
                                string outcome = tryMatchAction(argReader.Argument(1),
                        ...
                        case "clone":
                            ...
                            if (cloneBlocks.Count > 0)
                            {
                                ...
                                cloneBlocks.Clear();
                                GridTerminalSystem.GetBlockGroupWithName("Target")?.GetBlocks(cloneBlocks);
                                if (cloneBlocks.Count > 0)
                                {
                                ...
                                {
                                    log.add("Received Clone command, but there is no Target " +
                        ...
                        case "populate":
                            ...
                            if (iniRead.TryParse(Me.CustomData, out parseResult))
                            {
                                ...
                                else if (iniRead.ContainsSection("Populate"))
                                {
                                    ...
                                    GridTerminalSystem.GetBlockGroupWithName("Target")?.GetBlocks(popBlocks);
                                    ...
                                    {
                                        log.add("Received Populate command, but there is no Target " +
                                            "block group on the grid.");
                                    }
                                }
                                else
                        ...
                        case "tacticalnuke":
                            if (argReader.Switch("confirm"))
                            {
                                ...
                                GridTerminalSystem.GetBlockGroupWithName("Target")?.GetBlocks(tacBlocks);
                                ...
                                {
                                    log.add("Received TacticalNuke command, but there is no Target " +
                                        "block group on the grid.");
                                }
                            }
                            else
                        ...
                        case "properties":
                            ...
                            GridTerminalSystem.GetBlockGroupWithName("Target")?.GetBlocks(propBlocks);
                            ...
                            {
                                log.add("Received Properties command, but there is no Target " +
                                    "block group on the grid.");
                            }
                            break;
                        case "definitions":
                            ...
                            GridTerminalSystem.GetBlockGroupWithName("Target")?.GetBlocks(defBlocks);
                            ...
                            {
                                log.add("Received Definitions command, but there is no Target " +
                                    "block group on the grid.");
                            }
                            break;
                        ...
                        case "autopopulate":
                            ...
                            bool isTargeted = argReader.Switch("target");
                            bool isInclude = argReader.Switch("include");
                            ...
                            if (isTargeted)
                            {
                                ...
                                if (targetGroup == null)
                                {
                                    log.add("Received AutoPopulate command with the -target flag set, " +
                            ...
                            filterBlocks(apBlocks, filteredBlocks, b => MyIni.HasSection(b.CustomData, tag));
                            ...
                            //switch is set.
                            if (!hasExistingConfig || argReader.Switch("confirm") || isInclude)
                            {
                                List<Tally> tallyList = tallies.ToList();
                                string outcome = autoPopulate(apBlocks, tallyList, iniReadWrite, isInclude);
                        ...
                        case "clear":
                            ...
                            //Get the blocks in the Target group from the grid
                            GridTerminalSystem.GetBlockGroupWithName("Target")?.GetBlocks(clearBlocks);
                            ...
                            //If there weren't any blocks in the grid's Target group, complain.
                            else
                            {
                                log.add("Received Clear command, but there is no Target " +
                                            "block group on the grid.");
                            }
                            break;
                        ...
                        //update them
                        case "resetreports":
                            foreach (IReportable reportable in reports)
                            { reportable.setProfile(); }
                            log.add($"Carried out ResetReports command, re-applying text surface " +
                                $"variables of {reports.Length} Reports.");
                            break;
        ...
        public void compute()
        {
            ...
            foreach (Trigger trigger in triggers)
            {
                ...
                if (actionRequired)
                {
                    log.add(tryTakeAction(targetSet, desiredState, desiredState ? "on" : "off",
        ...
        public string tryMatchAction(string actionTarget, string actionCommand, string source)
        {
            ...
            else if (sets.TryGetValue(actionTarget, out targetSet))
            {
        ...
        public string tryTakeAction(ActionSet targetSet, bool desiredState, string command, string source)
        {
            ...
            foreach (ActionSet set in sets.Values)
            { set.resetHasActed(); }
        ...
        public string populatePB(string pbConfig, MyIni configReader, out string outcome)
        {
            ...
            {
                tryGetTriggerFromConfig(configReader, null, tallyDic, sets, "Populate", ID, iniValue,
        ...
        */
            ...
            MyIni configWriter, bool includeExistingItemTallies = false)
        {
            ...
            configureTallyGeneric("Power", new BatteryHandler(), filteredBlocks, tallyDic, tallyList, configWriter,
                reportElements, ref tallyCounter, ref replacementCounter, ref blockCounter);
            ...
            newTally = configureTallyGeneric("Solar", new PowerProducerHandler(), filteredBlocks, tallyDic,
                tallyList, configWriter, reportElements, ref tallyCounter, ref replacementCounter,
                ref blockCounter);
            ...
            configureTallyGeneric("JumpCharge", new JumpDriveHandler(), filteredBlocks, tallyDic, tallyList,
            ...
            configureTallyGeneric("Hydrogen", new GasHandler(), hyTanks, tallyDic, tallyList, configWriter,
                reportElements, ref tallyCounter, ref replacementCounter, ref blockCounter, false);
            configureTallyGeneric("Oxygen", new GasHandler(), oxyTanks, tallyDic, tallyList, configWriter,
                reportElements, ref tallyCounter, ref replacementCounter, ref blockCounter, false);
            ...
            configureTallyCargo("Cargo", false, ironOre, -1, filteredBlocks.Count, filteredBlocks,
                tallyDic, tallyList, cargoTallies, reportElements, ref tallyCounter);
            ...
            configureTallyCargo("Ore", false, ironOre, -1, filteredBlocks.Count, filteredBlocks,
                tallyDic, tallyList, cargoTallies, reportElements, ref tallyCounter, "", false);
            //So is the Stone tally.
            configureTallyCargo("Stone", true, new MyItemType(ORE_TYPE, "Stone"), 1000, filteredBlocks.Count,
                filteredBlocks, tallyDic, tallyList, cargoTallies, reportElements, ref tallyCounter);
            ...
            newTally = configureTallyCargo("GatlingAmmo", true, gatlingAmmo, 10, gatlingCount, null,
                tallyDic, tallyList, cargoTallies, reportElements, ref tallyCounter, "Gatling\nDrums", false);
            newTally = configureTallyCargo("AutocannonAmmo", true, autoAmmo, 10, autoCount, null,
                tallyDic, tallyList, cargoTallies, reportElements, ref tallyCounter, "Autocannon\nMagazines", false);
            newTally = configureTallyCargo("AssaultAmmo", true, assaultAmmo, 5, assaultCount, null,
                tallyDic, tallyList, cargoTallies, reportElements, ref tallyCounter, "Assault\nShells", false);
            newTally = configureTallyCargo("ArtilleryAmmo", true, artilleryAmmo, 3, artilleryCount, null,
                tallyDic, tallyList, cargoTallies, reportElements, ref tallyCounter, "Artillery\nShells", false);
            newTally = configureTallyCargo("RailSmallAmmo", true, railSmallAmmo, 5, railSmallCount, null,
                tallyDic, tallyList, cargoTallies, reportElements, ref tallyCounter, "Railgun\nS. Sabot", false);
            newTally = configureTallyCargo("RailgunLargeAmmo", true, railLargeAmmo, 5, railLargeCount, null,
                tallyDic, tallyList, cargoTallies, reportElements, ref tallyCounter, "Railgun\nL. Sabot", false);
            newTally = configureTallyCargo("RocketAmmo", true, rocketAmmo, 4, rocketCount, null,
            ...
            foreach (IMyTerminalBlock block in filteredBlocks)
            {
                ...
                if (configWriter.ContainsSection(tag))
                {
        ...
        public TallyGeneric configureTallyGeneric(string tallyName, ITallyGenericHandler handler,
            ...
            List<Tally> tallyList, MyIni configWriter, List<string> reportElements,
            ref int tallyCounter, ref int replacementCounter, ref int blockCounter, bool clearFiltered = true)
        {
            ...
            if (filteredBlocks.Count > 0)
            {
                ...
                {
                    tally = new TallyGeneric(meterMaid, tallyName, handler);
                    tallyList.Add(tally);
                    tallyCounter++;
                }
                else
        ...
        public TallyCargo configureTallyCargo(string tallyName, bool isItem, MyItemType type,
            int perBlock, int blockCount, List<IMyTerminalBlock> filteredBlocks,
            Dictionary<string, Tally> tallyDic, List<Tally> tallyList, List<TallyCargo> cargoTallies,
            List<string> reportElements, ref int tallyCounter, string displayName = "",
            bool clearFiltered = true)
        {
            ...
            if (blockCount > 0)
            {
                ...
                if (!tallyDic.ContainsKey(tallyName))
                {
                    ...
                    if (isItem)
                    { tally = new TallyItem(meterMaid, tallyName, type, perBlock * blockCount); }
                    else
        ...
        public bool initiate()
        {
            ...
            //an entry.
            double lastVersion = iniReadWrite.Get("Data", "Version").ToDouble(-1);
            //Try to pull the ID from the Config section of the Storage string, using the default 
            //ID if nothing is found.
            customID = iniReadWrite.Get("Data", "ID").ToString(DEFAULT_ID);
        ...
        public void evaluate(bool firstRun = false)
        {
            ...
            distributor.setDelay(iniRead.Get("Data", "UpdateDelay").ToInt32(0));
            ...
            while (counter != -1)
            {
                tally = tryGetTallyFromConfig(iniReadWrite, initTag, counter, iniValue, textLog);
                if (tally == null)
            ...
            while (counter != -1)
            {
                set = tryGetActionSetFromConfig(iniReadWrite, iniRead, initTag, counter, iniValue,
                    textLog);
                if (set == null)
            ...
            while (counter != -1)
            {
                ...
                tryGetTriggerFromConfig(iniReadWrite, iniRead, evalTallies, sets, initTag, counter, iniValue,
            ...
            //foreach (string loadedSet in loadedActionSets)
            {
                ...
                readPBPlansFromConfig(sets, evalTriggers, iniReadWrite, initTag,
                    i, set, iniValue, textLog);
            }

            //If we don't have errors, but we also don't have any tallies or ActionSets...
            if (textLog.getErrorTotal() == 0 && evalTallies.Count == 0 && sets.Count == 0)
                            ...
                            if (iniReadWrite.ContainsSection(discreetTag))
                            {
                                ...
                                if (actionPlan == null || !actionPlan.hasAction())
                                {
                                    ...
                                    if (iniReadWrite.ContainsKey(discreetTag, "Action0Property"))
                                    {
                                        ...
                                        while (counter != -1)
                                        {
                                            retreivedPart = tryGetPartFromConfig(textLog, discreetTag,
                                    ...
                                    else
                                    {
                                        ...
                                        blockPlan.actionOn = retrieveActionHandler(textLog, discreetTag,
                                            "ActionOn", block, iniReadWrite, actions);
                                        //Check to see if there's an ActionOff in the ACTION SECTION
                                        blockPlan.actionOff = retrieveActionHandler(textLog, discreetTag,
                                ...
                                {
                                    textLog.addError($"Block '{block.CustomName}', discreet section '{discreetTag}', " +
                ...
                if (parseResult.Success && block is IMyTextSurfaceProvider)
                {
                    ...
                    for (int i = 0; i < surfaceProvider.SurfaceCount; i++)
                    {
                        ...
                        else
                        {
                            ...
                            reportable = tryGetReportableFromConfig(textLog, $"Surface{i}", tag,
            ...
            triggers = evalTriggers.Values.ToArray();
            ...
            */
            ...
            if (textLog.getErrorTotal() > 0)
            {
                ...
                foreach (WallOText logReport in logReports)
                { logReport.update(); }
            }
            else
            {
                ...
                Runtime.UpdateFrequency = UpdateFrequency.Update100;
                //Also, brag.
                outcome += $"Script is now running. Registered {tallies.Length} tallies, " +
                    $"{sets.Count} ActionSets, {triggers.Length} triggers, and {reports.Length} " +
                    $"reports, as configured by data on {blocks.Count} blocks. Evaluation used " +
                    $"{Runtime.CurrentInstructionCount} / {Runtime.MaxInstructionCount} " +
                    $"({(int)(((double)Runtime.CurrentInstructionCount / Runtime.MaxInstructionCount) * 100)}%) " +
        ...
        private bool isElementNameInUse(HashSet<string> elementNames, string name, string descriptor,
            int index, LimitedMessageLog textLog)
        {
            ...
            {
                textLog.addError($"{descriptor} {index} tried to use the Element name '{name}', " +
            ...
            else if (elementNames.Contains(name))
            {
                textLog.addError($"{descriptor} {index} tried to use the Element name '{name}', " +
                    ...
                    $"Trigger) must have their own, unique names.");
                return true;
            }
            ...
            MyIniValue iniValue, LimitedMessageLog textLog)
        {
            ...
            if (!string.IsNullOrEmpty(setName))
            {
                ...
                if (tryGetColorFromConfig(textLog, ref color, $"Action{index}", "ColorOn",
                    targetSection, configReader, Me))
                { set.colorOn = color; }
                if (tryGetColorFromConfig(textLog, ref color, $"Action{index}", "ColorOff",
                    targetSection, configReader, Me))
                { set.colorOff = color; }
            ...
            int index, ActionSet set, MyIniValue iniValue, LimitedMessageLog textLog)
        {
            ...
            if (delayOn != 0 || delayOff != 0)
            {
                ...
                ActionPlanUpdate updatePlan = new ActionPlanUpdate(distributor);
            ...
            if (!iniValue.IsEmpty)
            {
                ...
                parseStateList(iniValue.ToString(), troubleID, textLog, parsedData);
        ...
        */
        public void parseStateList(string stateList, string troubleID, LimitedMessageLog textLog,
            List<KeyValuePair<string, bool>> parsedData)
        {
            ...
            foreach (string pair in pairs)
            {
                ...
                {
                    badState = true;
                    textLog.addError($"{troubleID} does not provide a state for the component " +
                ...
                {
                    badState = true;
                    textLog.addError($"{troubleID} attempts to set '{target}' to the invalid state " +
        ...
        public Action<IMyTerminalBlock> retrieveActionHandler(LimitedMessageLog textLog, string discreetTag, string target,
         ...
         */
        public ActionPart tryGetPartFromConfig(LimitedMessageLog textLog, string discreetTag, int index,
            IMyTerminalBlock block, MyIni iniReader, MyIniValue iniValue)
        {
            ...
            if (!iniValue.IsEmpty)
            {
                ...
                else if (propertyDef.TypeName.ToLowerInvariant() == "color")
                {
                    ...
                    if (tryGetColorFromConfig(textLog, ref typedValue, $"Action{index}", "ValueOn",
                        discreetTag, iniReader, block))
                    { typedPart.setValueOn(typedValue); }
                    if (tryGetColorFromConfig(textLog, ref typedValue, $"Action{index}", "ValueOff",
                        discreetTag, iniReader, block))
                    { typedPart.setValueOff(typedValue); }
        ...
        private IReportable tryGetReportableFromConfig(LimitedMessageLog textLog, string prefix, string sectionTag,
            IMyTextSurface surface, IMyTerminalBlock block, MyIni iniReader, Dictionary<string, Tally> evalTallies,
            Dictionary<string, ActionSet> sets, Dictionary<string, Trigger> evalTriggers)
        {
            MyIniValue iniValue;
        ...
        //  purposes.
        public bool tryGetColorFromConfig(LimitedMessageLog textLog, ref Color color, string prefix,
                ...
                if (!Hammers.tryGetColorFromElements(ref color, elements))
                {
                    ...
                    switch (elements[0].ToLowerInvariant())
                    {
                        ...
                        default:
                            ...
                            textLog.addError($"Block '{block.CustomName}', section {sectionTag}, prefix " +
        ...
        public class LimitedMessageLog
        {
            ...
            List<string> errors;
            //A list holding any notifications we've logged
            List<string> notes;

            public LimitedMessageLog(StringBuilder _sb, int maxErrors)
            {
                this._sb = _sb;
            ...
            //Return the total number of notes stored in this log
            public int getNoteTotal()
            { return notes.Count; }

            //Print a formatted list of notes in this log
        ...
        public class TallyItem : TallyCargo
        {
            ...
            public TallyItem(Hammers.MeterMaid meterMaid, string name, MyItemType itemType, double max = 0,
                bool isLow = false, double multiplier = 1) : base(meterMaid, name, isLow, multiplier)
            {
        ...
        public class Trigger : IHasElement, IHasConfig
        {
            ...
            {
                string config = $"Trigger{index}Name = {programName}\n";
                config += $"Trigger{index}Tally = {targetTally.programName}\n";
        ...
        public class ActionPlanUpdate : IHasActionPlan, IHasConfig
        {
            ...
            public string writeConfig(int index)
            {
                ...
                { config += $"Action{index}DelayOn = {delayOn}\n"; }
                if (delayOff != DEFAULT_DELAY_OFF)
                { config += $"Action{index}DelayOff = {delayOff}\n"; }
                return config;
            }
        }
        ...
        public class ActionPlanIGC : IHasActionPlan, IHasConfig
        {
            ...
            public string writeConfig(int index)
            {
                ...
                string config = "";
                if (channel != DEFAULT_IGC_CHANNEL)
                { config += $"Action{index}IGCChannel = {channel}\n"; }
                if (messageOn != DEFAULT_IGC_MESSAGE_ON)
                { config += $"Action{index}IGCMessageOn = {messageOn}\n"; }
                if (messageOff != DEFAULT_IGC_MESSAGE_OFF)
                { config += $"Action{index}IGCMessageOff = {messageOff}\n"; }
                return config;
            }
        }
