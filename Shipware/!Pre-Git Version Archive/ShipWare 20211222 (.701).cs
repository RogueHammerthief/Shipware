    ...
    partial class Program : MyGridProgram
    {
        ...
        public string tryTakeAction(string actionTarget, string actionCommand, string source)
        {
            ...
            else if (sets.ContainsKey(actionTarget))
            {
                ...
                catch (InvalidCastException err)
                {
                    ...
                    if (err.Data.Contains("Identifier"))
                    { identifier = $"{err.Data["Identifier"]}"; }
        ...
        public string populatePB(string pbConfig, MyIni configReader, out string outcome)
        {
            ...
            else if (type == "Action")
            {
                ...
                newAction = tryGetActionFromConfig(configReader, configReader, "Populate",
                    ID, iniValue, textLog, out initiatorName);
        ...
        public void evaluate(bool firstRun = false)
        {
            ...
            while (counter != -1)
            {
                ...
                {
                    textLog.addError($"The Tally name '{elementName}' is reserved by the script to indicate" +
                          $"where portions of the screen should be left empty. Please choose a " +
                          $"different name.");
                ...
                {
                    textLog.addError($"The Tally name '{elementName}' is already in use. Tallies and " +
            ...
            while (counter != -1)
            {
                ...
                {
                    textLog.addError($"The Action name '{elementName}' is reserved by the script to indicate" +
                          $"where portions of the screen should be left empty. Please choose a " +
                          $"different name.");
                    break;
                }
                ...
                {
                    textLog.addError($"The ActionSet name '{elementName}' is already in use. Tallies and " +
                            ...
                            if (iniReadWrite.ContainsSection(discreetTag))
                            {
                                ...
                                if (actionPlan == null || !actionPlan.hasAction())
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        blockPlan.actionOn = matchAction(textLog, discreetTag,
                                            "ActionOn", block, iniReadWrite, actions);
                                        //Check to see if there's an ActionOff in the ACTION SECTION
                                        blockPlan.actionOff = matchAction(textLog, discreetTag,
        ...
        private Tally tryGetTallyFromConfig(MyIni configReader, string targetSection, int index,
            MyIniValue iniValue, LimitedMessageLog textLog, out string tallyName)
        {
            Tally tally = null;
            string tallyType;
        ...
        private ActionSet tryGetActionFromConfig(MyIni configReader, MyIni saveReader, string targetSection, int index,
            MyIniValue iniValue, LimitedMessageLog textLog, out string actionName)
        {
            ActionSet set = null;
            Color color = Hammers.cozy;
            ...
            if (!string.IsNullOrEmpty(actionName))
            {
                ...
                if (!iniValue.IsEmpty)
                {
                    ...
                    {
                        textLog.addError($"Action '{actionName}' has configuration for sending an IGC " +
        ...
        public Action<IMyTerminalBlock> matchAction(LimitedMessageLog textLog, string discreetTag, string target,
            ...
            ref Tally targetTally, ref ActionSet targetSet, ref Trigger trigger, LimitedMessageLog textLog)
        {
            ...
            if (!string.IsNullOrEmpty(tallyName))
            {
                ...
                { textLog.addError($"Trigger {index} tried to reference the unconfigured Tally '{tallyName}'."); }
            }
            else
            { textLog.addError($"Trigger {index} has a missing or unreadable Tally."); }
            ...
            if (!string.IsNullOrEmpty(setName))
            {
                ...
                { textLog.addError($"Trigger {index} tried to reference the unconfigured ActionSet '{setName}'."); }
            }
            else
            { textLog.addError($"Trigger {index} has a missing or unreadable ActionSet."); }
                ...
                if (!iniValue.IsEmpty)
                {
                    ...
                    if (evalSets.ContainsKey(setName))
                    {
                        ...
                        if (!iniValue.IsEmpty)
                        {
                            ...
                            {
                                textLog.addError($"Trigger {index} uses the unknown command '{linkString}' " +
                        ...
                        {
                            textLog.addError($"Trigger {index} links to ActionSet '{setName}', but does " +
                                $"not define a valid ActionOn or ActionOff.");
                        }
                    }
                    else
                    {
                        textLog.addError($"Trigger {index} tried to reference the unconfigured ActionSet " +
                            $"'{setName}' as a target of LinkedActionSet.");
                    }
                }
                if (!trigger.hasScenario())
                { textLog.addError($"Trigger {index} does not define a valid LessOrEqual or GreaterOrEqual scenario."); }
            }
