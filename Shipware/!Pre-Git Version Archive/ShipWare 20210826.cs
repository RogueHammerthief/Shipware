...
namespace IngameScript
{
    ...
    public class ShieldAPI
    {
        ...
        public bool worldHasShields { get; private set; }
        ...
        public ShieldAPI(IMyTerminalBlock block)
        {
            ...
            if (delegates == null)
            {
                ...
                worldHasShields = false;
                return;
            }
            else
            { worldHasShields = true; }
    ...
    partial class Program : MyGridProgram
    {
        ...
        //An instance of the API that interfaces with the DefenseShields mod.
        ShieldAPI shieldAPI;
        ...
        public string autoPopulate(IMyGridTerminalSystem GTS, ref List<Tally> tallyList, MyIni configWriter)
        {
            ...
            Tally newTally;
            ITallyGenericHandler newHandler;
            //We won't know what tallies we'll need to submit information to until we've scanned the
            //grid. So we'll build tally listings for our inventory blocks as we go along.
            string cargoValue = "";
            string oreValue = "";
            string ingotValue = "";
            ...
            /*
            //Hydrogen. Because all GasTanks share the same interface, we need to make sure our 
            //prospective tanks are actually intended to hold hydrogen.
            List<IMyGasTank> hyTanks = new List<IMyGasTank>();
            GridTerminalSystem.GetBlocksOfType<IMyGasTank>(hyTanks,
                b => b.IsSameConstructAs(Me)
                && (b.BlockDefinition.SubtypeId == "LargeHydrogenTank"
                || b.BlockDefinition.SubtypeId == "LargeHydrogenTankSmall"
                || b.BlockDefinition.SubtypeId == "LargeHydrogenTankIndustrial"
                || b.BlockDefinition.SubtypeId == "SmallHydrogenTank"
                || b.BlockDefinition.SubtypeId == "SmallHydrogenTankSmall"));
            //Oxygen. Because all gas tanks share the same TypeID, and large grid oxygen tanks 
            //apparently don't have a SubTypeId, we define oxygen tanks as "The Gas Tanks that 
            //aren't Hydrogen Tanks".
            List<IMyGasTank> oxyTanks = new List<IMyGasTank>();
            GridTerminalSystem.GetBlocksOfType<IMyGasTank>(oxyTanks,
                b => b.IsSameConstructAs(Me)
                && b.BlockDefinition.SubtypeId != "LargeHydrogenTank"
                && b.BlockDefinition.SubtypeId != "LargeHydrogenTankSmall"
                && b.BlockDefinition.SubtypeId != "LargeHydrogenTankIndustrial"
            ...
            */
            ...
            List<IMyUserControllableGun> gatlings = new List<IMyUserControllableGun>();
            ...
            if (gatlings.Count > 0)
            {
                ...
                componentValue = $"{componentValue}Drums, ";
            }
            ...
            if (rockets.Count > 0)
            {
                ...
                componentValue = $"{componentValue}Rockets, ";
            }
            //H2/02 generators.
            List<IMyGasGenerator> iceCrackers = new List<IMyGasGenerator>();
            ...
            //Our inventory configs should be set at this. As a last step, we'll trim the trailing
            //comma and space, checking to make sure the strings aren't empty
            ...
            { componentValue = componentValue.Remove(componentValue.Length - 2); }
            ...
            /*
            {
                ...
                {
                    newHandler = new BatteryHandler();
                    newTally = new TallyGeneric(meterMaid, tallyName, newHandler);
                    tallyList.Add(newTally);
                    tallyCounter++;
                }
                foreach (IMyTerminalBlock block in batteries)
                {
            ...
            /*
            {
                ...
                if (!tallyDic.ContainsKey(tallyName))
                {
                    newHandler = new GasHandler();
                    newTally = new TallyGeneric(meterMaid, tallyName, newHandler);
                    tallyList.Add(newTally);
                    tallyCounter++;
                }
                foreach (IMyTerminalBlock block in hyTanks)
                {
                    if (MyIni.HasSection(block.CustomData, tag))
                    {
                        configWriter.TryParse(block.CustomData);
        ...
        */
            ...
            /*  {
                    ...
                    foreach (IMyTerminalBlock block in oxyTanks)
                    {
                        if (MyIni.HasSection(block.CustomData, tag))
                        {
                            configWriter.TryParse(block.CustomData);
            ...
            /*
            {
                ...
                foreach (IMyTerminalBlock block in cargo)
                {
                    if (MyIni.HasSection(block.CustomData, tag))
                    {
                        configWriter.TryParse(block.CustomData);
                    ...
                    //We've already generated the full cargoConfig. No section or key needed.
                    block.CustomData = $"{block.CustomData}\n\n{cargoValue}";
                    blockCounter++;
                }
            }*/
            ...
            {
                tallyName = "Drums";
                newTally = new TallyItem(meterMaid, tallyName, "MyObjectBuilder_AmmoMagazine", "NATO_25x184mm", max);
                //Drums 
                configureTally(configWriter, tallyDic, tallyName, tallyName, newTally, oxyTanks,
                    ref tallyList, ref tallyCounter, ref replacementCounter, ref blockCounter);
            }
            /*
            {
                ...
                foreach (IMyTerminalBlock block in gatlings)
                {
                    if (MyIni.HasSection(block.CustomData, tag))
                    {
                        configWriter.TryParse(block.CustomData);
            ...
            if (rockets.Count > 0)
            {
                ...
                if (!tallyDic.ContainsKey(tallyName))
                {
                    ...
                    newTally = new TallyItem(meterMaid, tallyName, "MyObjectBuilder_AmmoMagazine", "Missile200mm", max);
                    tallyList.Add(newTally);
                    tallyCounter++;
                }
                foreach (IMyTerminalBlock block in rockets)
                {
                    if (MyIni.HasSection(block.CustomData, tag))
                    {
                        configWriter.TryParse(block.CustomData);
            ...
            if (iceCrackers.Count > 0)
            {
                ...
                if (!tallyDic.ContainsKey(tallyName))
                {
                    ...
                    newTally = new TallyItem(meterMaid, tallyName, "MyObjectBuilder_Ore", "Ice", max);
                    tallyList.Add(newTally);
                    tallyCounter++;
                }
                foreach (IMyTerminalBlock block in iceCrackers)
                {
                    if (MyIni.HasSection(block.CustomData, tag))
                    {
                        configWriter.TryParse(block.CustomData);
            ...
            {
                if (!tallyDic.ContainsKey("Ore"))
                {
                    newTally = new TallyCargo(meterMaid, "Ore");
                    tallyList.Add(newTally);
                    tallyCounter++;
                }
                if (!tallyDic.ContainsKey("Stone"))
                {
                    max = (refineries.Count + drills.Count) * 1000;
                    newTally = new TallyItem(meterMaid, "Stone", "MyObjectBuilder_Ore", "Stone", max, true);
                    tallyList.Add(newTally);
                    tallyCounter++;
                }
                //For the sake of less duplicate code, we'll make an Ore list to store both 
                //refineries and drills
                //TODO: Monitor. I'm actually just lumping the drills into the refineries list 
                //here, as opposed to creating a new, unified list. Right now, that isn't a problem,
                //because I'm not actually using either list for anything else.
                //I'm doing the same thing later with smallPanels and largePanels
                List<IMyTerminalBlock> oreHolders = refineries;
                oreHolders.AddList(drills);
                foreach (IMyTerminalBlock block in oreHolders)
                {
                    if (MyIni.HasSection(block.CustomData, tag))
                    {
                        configWriter.TryParse(block.CustomData);
            ...
            if (reactors.Count > 0)
            {
                ...
                if (!tallyDic.ContainsKey(tallyName))
                {
                    ...
                    newTally = new TallyItem(meterMaid, tallyName, "MyObjectBuilder_Ingot", "Uranium", max);
                    tallyList.Add(newTally);
                    tallyCounter++;
                }
                foreach (IMyTerminalBlock block in reactors)
                {
                    if (MyIni.HasSection(block.CustomData, tag))
                    {
                        configWriter.TryParse(block.CustomData);
            ...
            if (smallPanels.Count > 0 || largePanels.Count > 0)
            {
                ...
                if (!tallyDic.ContainsKey(tallyName))
                {
                    ...
                    newHandler = new PowerProducerHandler();
                    newTally = new TallyGeneric(meterMaid, tallyName, newHandler, false, 1000);
                    newTally.forceMax(max);
                    tallyList.Add(newTally);
                    tallyCounter++;
                }
                List<IMySolarPanel> panels = smallPanels;
                panels.AddList(largePanels);
                foreach (IMyTerminalBlock block in panels)
                {
                    if (MyIni.HasSection(block.CustomData, tag))
                    {
                        configWriter.TryParse(block.CustomData);
            ...
            if (jumpDrives.Count > 0)
            {
                ...
                if (!tallyDic.ContainsKey(tallyName))
                {
                    newHandler = new JumpDriveHandler();
                    newTally = new TallyGeneric(meterMaid, tallyName, newHandler);
                    newTally.displayName = "Jump Charge";
                    tallyList.Add(newTally);
                    tallyCounter++;
                }
                foreach (IMyTerminalBlock block in jumpDrives)
                {
                    if (MyIni.HasSection(block.CustomData, tag))
                    {
                        configWriter.TryParse(block.CustomData);
            ...
            //Instead, we ask the API if there's a shield.
            //Note: This will return empty for a while after world load, even if the shield mod is 
            //loaded and the grid has a shield. Shouldn't be a problem with AutoPopulate, though.
            if (shieldAPI.ShieldStatus() != string.Empty)
            {
                tallyName = "ShieldHealth";
                if (!tallyDic.ContainsKey(tallyName))
                {
                    newTally = new TallyShield(meterMaid, tallyName, new ShieldHealthHandler(shieldAPI));
                ...
                if (!tallyDic.ContainsKey(tallyName))
                {
                    newTally = new TallyShield(meterMaid, tallyName, new ShieldHeatHandler(shieldAPI), true);
                ...
                if (!tallyDic.ContainsKey(tallyName))
                {
                    newTally = new TallyShield(meterMaid, tallyName, new ShieldRegenHandler(shieldAPI));
            ...
            //We'll need a list of existing Tallies
            string reportElements = "";
            foreach (Tally tally in tallyList)
            { reportElements = $"{reportElements}, {tally.programName}"; }
            //Trim the leading comma and space
            reportElements = reportElements.Remove(0, 2);
            Me.CustomData =
                $"[{tag}]\n" +
                $"Surface0Elements = {reportElements}\n" +
                $"Surface0Columns = {(tallyList.Count < 3 ? $"{tallyList.Count}" : "3")}\n" +
        ...
        public void initiate()
        {
            ...
            //Try to initialize the ShieldAPI. If this world doesn't have shields, the API knows how
            //to handle that.
            shieldAPI = new ShieldAPI(Me);
            ...
            MyIniValue iniValue, LimitedErrorLog errors, out string tallyName)
        {
            ...
            if (!string.IsNullOrEmpty(tallyName))
            {
                ...
                {
                    if (shieldAPI.worldHasShields)
                    { tally = new TallyShield(meterMaid, tallyName, new ShieldHealthHandler(shieldAPI)); }
                    else
                ...
                {
                    if (shieldAPI.worldHasShields)
                    { tally = new TallyShield(meterMaid, tallyName, new ShieldHeatHandler(shieldAPI)); }
                    else
                    { errors.add(errorShieldMissing + tallyName); }
                }
                ...
                {
                    if (shieldAPI.worldHasShields)
                    { tally = new TallyShield(meterMaid, tallyName, new ShieldRegenHandler(shieldAPI)); }
                    else
                    { errors.add(errorShieldMissing + tallyName); }
                }
        ...
        public class TallyShield : Tally
        {
            ...
            internal ITallyShieldHandler handler;

            //TallyShield is built along the same lines as TallyGeneric, so the constructor is 
            //practically identical.
            public TallyShield(Hammers.MeterMaid meterMaid, string name, ITallyShieldHandler handler,
                bool isLow = false, double multiplier = 1) : base(meterMaid, name, isLow, multiplier)
            {
                this.handler = handler;
            }
        ...
        //Volume, Item, Oxygen, Hydrogen, Power, JumpDrive, Raycast, Max Output (Solar/Wind),
        //HydrogenWithEngines?, ShieldIntegrity?
        public static double handleBattery(List<IMyTerminalBlock> blocks)
        {
            double curr = 0;
        ...
        public static double handleGas(List<IMyTerminalBlock> blocks)
        {
            double curr = 0;
        ...
        public static double handleJumpCharge(List<IMyTerminalBlock> blocks)
        {
            double curr = 0;
        ...
        public static double handleRaycaster(List<IMyTerminalBlock> blocks)
        {
            double curr = 0;
        ...
        public static double handlePowerProducer(List<IMyTerminalBlock> blocks)
        {
            double curr = 0;
        ...
        //An interface for bespoke objects that store and manage specific block types for TallyGenerics
        public interface ITallyShieldHandler
        {
            ...
            void getUpdate(out double curr, out double max, out double ratio, out string trouble);
            //Return the string that will configure tallies of this type.
            string writeConfig();
        }

        public class ShieldHealthHandler : ITallyShieldHandler
        {
            //A reference to the ShieldAPI
            ShieldAPI api;

            public ShieldHealthHandler(ShieldAPI api)
            { this.api = api; }

            public void getUpdate(out double curr, out double max, out double ratio, out string trouble)
            {
                if (api.ShieldStatus() == string.Empty)
                {
                    curr = -1;
                    max = -1;
                    ratio = 0;
                ...
                {
                    curr = api.GetCharge() * api.HpToChargeRatio();
                    max = api.GetMaxCharge() * api.HpToChargeRatio();
                    ratio = curr / max;
                    trouble = "";
                }
            }
        ...
        public class ShieldHeatHandler : ITallyShieldHandler
        {
            ShieldAPI api;

            public ShieldHeatHandler(ShieldAPI api)
            { this.api = api; }
            ...
            public void getUpdate(out double curr, out double max, out double ratio, out string trouble)
            {
                ...
                {
                    curr = api.GetShieldHeat();
                    max = ShieldAPI.HEAT_MAX;
                    ratio = curr / max;
                    trouble = "";
                }
            }

            //The maximum heat of the shield system
            public double getMax()
            { return ShieldAPI.HEAT_MAX; }

            //The current heat of the shield system
            public double getCurr()
        ...
        public class ShieldRegenHandler : ITallyShieldHandler
        {
            //A reference to the ShieldAPI
            ShieldAPI api;

            public ShieldRegenHandler(ShieldAPI api)
            { this.api = api; }

            public void getUpdate(out double curr, out double max, out double ratio, out string trouble)
            {
                //If there's no shield system on the grid, ShieldStatus will return an empty string
                if (api.ShieldStatus() == string.Empty)
                {
                    curr = -1;
                    max = -1;
                ...
                //When the shield is fully charged, the charge rate returns 0. We need to handle that.
                ...
                {
                    curr = api.GetChargeRate() * api.HpToChargeRatio();
                    max = api.GetChargeRate() * api.HpToChargeRatio() /
