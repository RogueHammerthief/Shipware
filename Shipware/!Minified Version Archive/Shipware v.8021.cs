 ...
 */
const double ό=.8021;const string ϋ="Shipware";const string ϊ="SW";const string ω="Dec";string ψ;string χ;const string φ
="Source";const string υ="Target";ǟ τ;MyIni σ;MyIni ς;StringBuilder Ɓ;Ǫ ρ;l[]ο;v[]ξ;Dictionary<string,Ơ>ν;Ȩ[]μ;Dictionary
<string,Ǻ>λ;ɧ[]κ;ǋ[]ι;Dictionary<string,ɫ>θ;List<ƻ>η;IMyBroadcastListener ζ;ġ ε;Ч π;Dictionary<string,Ч>ύ;bool ϕ;DateTime
ϟ;string ϝ;static Color Ϝ=new Color(25,225,100);static Color ϛ=new Color(100,200,225);static Color Ϛ=new Color(255,255,0)
;static Color ϙ=new Color(255,150,0);static Color Ϙ=new Color(255,0,0);static Color ϗ=new Color(255,225,200);static Color
ϖ=new Color(0,0,0);Program(){Ƃ Ě;bool ϐ;Ϟ(out Ě,out ϐ);if(ϐ){ϓ(Ě);}else{ҁ(Ě);}Echo(τ.ǫ());}void Ϟ(out Ƃ Ě,out bool ϐ){σ=
new MyIni();ς=new MyIni();Ɓ=new StringBuilder();ϕ=false;ϟ=DateTime.Now;Ě=new Ƃ(Ɓ,15);ϐ=false;σ.TryParse(Storage);double ϔ=σ
.Get("Data","Version").ToDouble(-1);ψ=σ.Get("Data","ID").ToString(ϋ);χ=$"{ϊ}.{ψ}";ζ=IGC.RegisterBroadcastListener(χ);ζ.
...
IMyPowerProducer Ʀ=P as IMyPowerProducer;if(Ʀ!=null&&(Ʀ.BlockDefinition.SubtypeId.EndsWith("HydrogenEngine")||Ʀ.BlockDefinition.
SubtypeId=="LargePrototechReactor")){ƨ.Add(Ʀ);return true;}else{return false;}}}public double Ǆ(){double R=0;string[]ļ;string[]ƥ=
{"(","L/","L)"};foreach(IMyGasTank Ƥ in Ʃ){R+=Ƥ.Capacity;}foreach(IMyTerminalBlock ƣ in ƨ){ļ=ƣ.DetailedInfo.Split(ƥ,
System.StringSplitOptions.None);R+=Double.Parse(ļ[2]);}return R;}public double ǃ(){double q=0;foreach(IMyGasTank Ƥ in Ʃ){q+=Ƥ.
Capacity*Ƥ.FilledRatio;}foreach(IMyTerminalBlock ƣ in ƨ){q+=ƣ.Components.Get<MyResourceSourceComponent>().RemainingCapacity;}
return q;}public string ǂ(){return"";}public string ũ(){return"Gas";}}class Ƣ:ǅ{List<IMyJumpDrive>ơ;public Ƣ(){ơ=new List<
IMyJumpDrive>();}public bool Q(IMyTerminalBlock P){IMyJumpDrive Ƭ=P as IMyJumpDrive;if(Ƭ==null){return false;}else{ơ.Add(Ƭ);return
true;}}public double Ǆ(){double R=0;foreach(IMyJumpDrive Ƶ in ơ){R+=Ƶ.MaxStoredPower;}return R;}public double ǃ(){double q=0
;foreach(IMyJumpDrive Ƶ in ơ){q+=Ƶ.CurrentStoredPower;}return q;}public string ǂ(){return"";}public string ũ(){return
"JumpDrive";}}class ƴ:ǅ{List<IMyCameraBlock>ơ;Ǻ Ƴ;public ƴ(){ơ=new List<IMyCameraBlock>();Ƴ=null;}public void Ʋ(Ǻ Ʊ){Ƴ=Ʊ;}public
bool Q(IMyTerminalBlock P){IMyCameraBlock Ƭ=P as IMyCameraBlock;if(Ƭ==null){return false;}else{ơ.Add(Ƭ);return true;}}public
double Ǆ(){return-1;}public double ǃ(){double q=0;foreach(IMyCameraBlock ư in ơ){q+=ư.AvailableScanRange;}return q;}public
string ǂ(){return Ƴ==null?"":$"Raycaster = {Ƴ.s}\n";}public string ũ(){return"Raycast";}}class Ư:ǅ{List<IMyPowerProducer>ơ;
public Ư(){ơ=new List<IMyPowerProducer>();}public bool Q(IMyTerminalBlock P){IMyPowerProducer Ƭ=P as IMyPowerProducer;if(Ƭ==
null){return false;}else{ơ.Add(Ƭ);return true;}}public double Ǆ(){double R=0;foreach(IMyPowerProducer Ʈ in ơ){R+=Ʈ.
Components.Get<MyResourceSourceComponent>().DefinedOutput;}return R;}public double ǃ(){double q=0;foreach(IMyPowerProducer Ʈ in ơ)
{q+=Ʈ.MaxOutput;}return q;}public string ǂ(){return"";}public string ũ(){return"PowerMax";}}class ƭ:ǅ{List<
IMyPowerProducer>ơ;public ƭ(){ơ=new List<IMyPowerProducer>();}public bool Q(IMyTerminalBlock P){IMyPowerProducer Ƭ=P as IMyPowerProducer
;if(Ƭ==null){return false;}else{ơ.Add(Ƭ);return true;}}public double Ǆ(){double R=0;foreach(IMyPowerProducer Ʈ in ơ){R+=Ʈ
.Components.Get<MyResourceSourceComponent>().DefinedOutput;}return R;}public double ǃ(){double q=0;foreach(
IMyPowerProducer Ʈ in ơ){q+=Ʈ.CurrentOutput;}return q;}public string ǂ(){return"";}public string ũ(){return"PowerCurrent";}}class ǰ:ǅ{
List<IMySlimBlock>ơ;public ǰ(){ơ=new List<IMySlimBlock>();}public bool Q(IMyTerminalBlock P){IMySlimBlock Ƭ=P.CubeGrid.
GetCubeBlock(P.Min);ơ.Add(Ƭ);return true;}public double Ǆ(){double R=0;foreach(IMySlimBlock P in ơ){R+=P.MaxIntegrity;}return R;}
public double ǃ(){double q=0;foreach(IMySlimBlock P in ơ){q+=P.BuildIntegrity-P.CurrentDamage;}return q;}public string ǂ(){
return"";}public string ũ(){return"Integrity";}}class ǯ:ǅ{List<IMyAirVent>ơ;public ǯ(){ơ=new List<IMyAirVent>();}public bool Q
(IMyTerminalBlock P){IMyAirVent Ƭ=P as IMyAirVent;if(Ƭ==null){return false;}else{ơ.Add(Ƭ);return true;}}public double Ǆ()
{double R=0;foreach(IMyAirVent Ǯ in ơ){R+=1;}return R;}public double ǃ(){double q=0;foreach(IMyAirVent Ǯ in ơ){q+=Ǯ.
GetOxygenLevel();}return q;}public string ǂ(){return"";}public string ũ(){return"VentPressure";}}class ǭ:ǅ{List<IMyPistonBase>ơ;public
ǭ(){ơ=new List<IMyPistonBase>();}public bool Q(IMyTerminalBlock P){IMyPistonBase Ƭ=P as IMyPistonBase;if(Ƭ==null){return
false;}else{ơ.Add(Ƭ);return true;}}public double Ǆ(){double R=0;foreach(IMyPistonBase Ǵ in ơ){R+=Ǵ.HighestPosition;}return R;
}public double ǃ(){double q=0;foreach(IMyPistonBase Ǵ in ơ){q+=Ǵ.CurrentPosition;}return q;}public string ǂ(){return"";}
public string ũ(){return"PistonExtension";}}class ǳ:ǅ{List<IMyMotorStator>ơ;public ǳ(){ơ=new List<IMyMotorStator>();}public
bool Q(IMyTerminalBlock P){IMyMotorStator Ƭ=P as IMyMotorStator;if(Ƭ==null){return false;}else{ơ.Add(Ƭ);return true;}}public
double Ǆ(){double R=0;foreach(IMyMotorStator ǲ in ơ){R+=360;}return R;}public double ǃ(){double q=0;foreach(IMyMotorStator ǲ
in ơ){q+=MathHelper.ToDegrees(ǲ.Angle);}return q;}public string ǂ(){return"";}public string ũ(){return"RotorAngle";}}class
ǵ:ǅ{List<IMyShipController>ơ;public ǵ(){ơ=new List<IMyShipController>();}public bool Q(IMyTerminalBlock P){
IMyShipController Ƭ=P as IMyShipController;if(Ƭ==null){return false;}else{ơ.Add(Ƭ);return true;}}public double Ǆ(){return 110;}public
double ǃ(){double q=-1;foreach(IMyShipController Ǡ in ơ){if(Ǡ.IsFunctional){q=Ǡ.GetShipSpeed();break;}}return q;}public string
ǂ(){return"";}public string ũ(){return"ControllerSpeed";}}class Ǳ:ǅ{List<IMyShipController>ơ;public Ǳ(){ơ=new List<
IMyShipController>();}public bool Q(IMyTerminalBlock P){IMyShipController Ƭ=P as IMyShipController;if(Ƭ==null){return false;}else{ơ.Add(Ƭ
);return true;}}public double Ǆ(){return 1;}public double ǃ(){double q=0;foreach(IMyShipController Ǡ in ơ){if(Ǡ.
IsFunctional){q=Ǡ.GetNaturalGravity().Length()/9.81;break;}}return q;}public string ǂ(){return"";}public string ũ(){return
"ControllerGravity";}}class Ǭ:ǅ{List<IMyShipController>ơ;public Ǭ(){ơ=new List<IMyShipController>();}public bool Q(IMyTerminalBlock P){
IMyShipController Ƭ=P as IMyShipController;if(Ƭ==null){return false;}else{ơ.Add(Ƭ);return true;}}public double Ǆ(){return-1;}public
double ǃ(){double q=-1;foreach(IMyShipController Ǡ in ơ){if(Ǡ.IsFunctional){q=Ǡ.GetNaturalGravity().Length()*Ǡ.
CalculateShipMass().PhysicalMass;break;}}return q;}public string ǂ(){return"";}public string ũ(){return"ControllerWeight";}}class ǟ{List<
string>Ɛ;string ǖ;public string Ǟ{private get;set;}public int ǝ{private get;set;}public string ǜ{private get;set;}
StringBuilder Ɓ;string ǡ;int ǔ;public bool ǚ{get;private set;}string[]Ǚ;int ǘ=-1;int Ǘ;public ǟ(StringBuilder Ɓ,string ǖ,bool Ǖ=false
,int ǔ=5){Ɛ=new List<string>();this.Ɓ=Ɓ;ǡ="";this.ǖ=ǖ;Ǟ="";ǜ="";this.ǔ=ǔ;ǚ=false;ǝ=0;if(Ǖ){Ǚ=new string[]{"|----","-|---"
,"--|--","---|-","----|"};ǘ=0;Ǘ=1;}}public void ģ(){ǘ+=Ǘ;if(ǘ==0||ǘ==4){Ǘ*=-1;}}public void Ǔ(string Ǜ){Ɛ.Insert(0,
$"{DateTime.Now.ToString("HH:mm:ss")}- {Ǜ}");if(Ɛ.Count>ǔ){Ɛ.RemoveAt(ǔ);}Ɓ.Clear();foreach(string ŧ in Ɛ){Ɓ.Append($"\n{ŧ}\n");}ǡ=Ɓ.ToString();Ɓ.Clear();ǚ=true;}
public void Ǧ(){ǚ=false;}public string ǫ(){Ɓ.Clear();Ɓ.Append(ǖ);if(ǘ!=-1){Ɓ.Append($" {Ǚ[ǘ]}");}Ɓ.Append("\n");if(!String.
IsNullOrEmpty(Ǟ)){Ɓ.Append($"Script Tag: {Ǟ}\n");}if(ǝ!=0){Ɓ.Append($"Current Update Delay: {ǝ}\n");}Ɓ.Append($"{ǜ}\n");Ɓ.Append(ǡ);
return Ɓ.ToString();}}class Ǫ{StringBuilder Ɓ;int ǩ;string[]Ǩ;public Ǫ(StringBuilder Ɓ,int Ǣ=10){this.Ɓ=Ɓ;ǩ=Ǣ;Ǩ=new string[ǩ+1
];string ǧ="";for(int ņ=0;ņ<Ǩ.Length;ņ++){Ǥ(ref ǧ,ņ,ǩ);Ǩ[ņ]=ǧ;}}public void Í(ref string Î,double µ){Î=Ǩ[ǥ(µ,ǩ)];}public
void Í(ref string Î,double µ,int Ǣ){int ǣ=ǥ(µ,Ǣ);Ǥ(ref Î,ǣ,Ǣ);}private int ǥ(double µ,int Ǣ){µ=Math.Min(µ,100);return(int)((
µ/100)*Ǣ);}private void Ǥ(ref string Î,int ǣ,int Ǣ){Ɓ.Clear();Ɓ.Append('[');for(int ņ=0;ņ<ǣ;++ņ){Ɓ.Append('|');}for(int ņ
=ǣ;ņ<Ǣ;++ņ){Ɓ.Append(' ');}Ɓ.Append(']');Î=Ɓ.ToString();Ɓ.Clear();}}